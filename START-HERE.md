...
`prd/registers/screens.md` §2 says which. **You skip every `V2` row.** They are real scope, but
...
|---|---|---|
| 1 | `ux/claude-design-context.md` | **Paste it.** Same file, every session, never edited. |
| 2 | `ux/briefs/SCR-….md` | **Paste it.** One per session. This is the screen. |
| 3 | `prd/registers/screens.md` | **Edit 1 line** when the screen is approved. |
| 4 | `tasks/<module>.md` | **Edit 1 line** when the screen is approved. |
...
`prd/registers/screens.md` §2. So "picking the brief" isn't a decision; it's a lookup.
...
Tier         : P0
Rows         : 8
Brief        : ux/briefs/SCR-SHELL-01-app-shell.md   ← this is the file you paste
...
Open `prd/registers/screens.md`, go to **§2 Screen index**, and scroll down to the **first row
...
1. **one row** in `prd/registers/screens.md` — the row you just read
2. **one `DESIGN:` line** in `tasks/<module>.md` — e.g. `tasks/SHELL.md` for the SHELL screens,
   `tasks/M06-proposals.md` for the proposal builder
...
1. the whole contents of `ux/claude-design-context.md`, unchanged
2. the whole contents of the brief named by the register — for screen one,
   `ux/briefs/SCR-SHELL-01-app-shell.md`
...
### Edit 1 — `prd/registers/screens.md` (screen one is line 160)

Change `pending` → `designed`, and `—` → your Claude Design link. Leave the `V` column alone:

```
BEFORE  | SCR-SHELL-01 | **App Shell & Navigation** | P0 | 8 | `ux/briefs/…` | V1 | pending | — | …
AFTER   | SCR-SHELL-01 | **App Shell & Navigation** | P0 | 8 | `ux/briefs/…` | V1 | designed | <link> | …
```
...
There is no equivalent count on the task side, because `tasks/` has no `V` column — the scope
lock lives in the register only. If you want the task-side view of *everything*, designed or not:

```bash
grep -rhoE 'DESIGN:?\*{0,2} *SCR-[A-Z0-9]+-[0-9]+ *→ *PENDING' tasks/ --include='*.md' --exclude=README.md | wc -l
```
