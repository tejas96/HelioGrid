...
import { CurrentClaims } from '../../common/decorators/current-claims.decorator';
import { AuthService } from './auth.service';

const TEAM_MANAGERS: readonly RolePreset[] = ['owner', 'manager'];
...
function requireTeamManager(claims: SessionClaims) {
  if (!claims.roles.some((r) => TEAM_MANAGERS.includes(r)))
    throw new ForbiddenException('Team management needs the owner or manager role.');
}

// No @UseGuards: SessionGuard is global (APP_GUARD, deny-by-default in app.module.ts).
@Controller()
