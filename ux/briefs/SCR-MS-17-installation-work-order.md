...
**Module:** MS (M05 Design Studio · studio sub-spec MS11) · **Personas:** Project Manager / Installation Coordinator (runs the checklist and attributes work — R16), Design Engineer, Operations · **Context of use:** the crew has no login in v1 (R16, prd/02-personas.md) — the coordinator runs the checklist, typically on a phone on site, one-handed, and attributes ticks with an optional free-text "done by"; the printed sheet goes to paper in the field, so print is a first-class output, not a screenshot; the Project Manager works the web board with mobile for stage moves (prd/02-personas.md persona table).
...
For awareness: the same ledger's law that no commercial figure ever reaches this surface (MS11-32, prd/modules/M05-studio/10-done-and-installation.md) — the work order carries quantities, never money.
