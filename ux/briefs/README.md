...
`prd/registers/screens.md`. That register is the single source of truth for what exists and
...
2. Paste `ux/claude-design-context.md` — unchanged, every session.
...
8. Record the result in **two** places — both, or the trail breaks:
   - `prd/registers/screens.md`: the screen's row, `UX status` → `designed`, `Design link` → the artifact.
   - `tasks/<module>.md`: that screen's `DESIGN: SCR-… → PENDING` line → the artifact link.
...
**`START-HERE.md` at the repo root is the fuller version of this loop**, with the exact messages
to send and worked before/after examples of both edits. If the two ever disagree, `START-HERE.md`
...
first, briefs are regenerated to match, and the affected engineering tasks in `tasks/` are
