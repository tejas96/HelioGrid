...
**Module:** M10 · HR-lite · **Personas:** HR/Admin (primary), EPC Owner · **Context of use:** this is the HR/Admin's home screen — a queue, not a report. Mobile for approvals and the exceptions list, which is the part that is genuinely time-sensitive (`prd/02-personas.md` §HR/Admin, Primary surfaces); the people-today queue, leave decisions and re-invites are one-tap mobile acts (`prd/modules/M10-hr-lite.md` §2). Web full-featured too (both surfaces full-featured).

## Entry & exit

Reached from: sign-in — this queue is the HR/Admin role home (`M13-37`, `PS-30`); the composition of the home screen is `modules/M13`'s, the facts and states it composes are M10's (`prd/modules/M10-hr-lite.md` §M10.1 behavior detail). Leads to: the Employee Record (SCR-M10-03) — "the record (§M10.2) is what the queue items resolve into"; the one-tap resend (the act is M01's, Owner-only) and the deep links to M01's Owner-gated Team screens; attendance exceptions are "each resolvable by looking at the person's own timeline" (M09's timeline, per `M10-26`); leave decisions land on the Attendance Register (SCR-M10-05); document attention items resolve on the record's documents (SCR-M10-03).
