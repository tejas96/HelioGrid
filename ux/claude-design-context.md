...
screen brief from `ux/briefs/`. The design system is already selected in Claude Design — nothing
in this file says how anything should look. Everything in this file is **product law**: what a
screen must do, carry and survive. Source of authority: `prd/foundations/F7-design-language.md`
(F7-23, F7-43), `prd/foundations/F8-data-honesty.md`.
...
5. **No orphan screens**: every screen is reachable and leads somewhere. Where the brief pins the
   ...
   **One qualification on item 5 — an owner decision, not your judgement.** The product has 150
   ...
   decision rather than an oversight. The `V` column in `prd/registers/screens.md` §2 is the
