...
| ID | Gap | Stage | Why it matters | Design-at-implementation notes | Blocks |
|---|---|---|---|---|---|
| UXG-A11Y-01 | **`--text-secondary` on `--canvas-sunken` measures ≈3.89:1** — used for the AvatarGroup "+N" overflow count at 12px/500 | any | Found 2026-07-30 by the tokens contrast-coverage gate (docs/17 §5), not by review. WCAG AA needs **4.5:1** for normal text; 12px/500 is not "large text" under any reading, so this fails AA. Ruling C (docs/10 §3.2) addresses `--text-secondary` on **white** (≈4.45:1) and does **not** cover the sunken surface. Nothing was previously checking this pairing at all. | **Owner ruling needed.** Recommended fix: render the overflow count in `--text-primary` on `--canvas-sunken` (already declared, ≈13:1, floor 7) — a one-token change in `packages/ui/src/data/Avatar.css` and its RN mirror, no layout impact. Alternatives: raise the count's size/weight into WCAG "large text" (≥18.66px bold), or keep the value and accept a documented AA exception. Until ruled, `DECLARED_PAIRS` pins the floor at the measured 3.8 so it cannot silently worsen, and the entry carries an `OPEN FINDING` restriction. | Any slice shipping AvatarGroup (`packages/ui`, `apps/mobile/src/ui`) |
