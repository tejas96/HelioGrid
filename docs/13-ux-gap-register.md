...
| UXG-05 | **Merge-customer flow** (husband/wife, two numbers) | 2–3 | Dedupe by phone cannot catch it; journey says "offer merge later" but the flow is undesigned (./research/journey.md §6.8) | **v1.1** per ruling R8 (docs/15). Data model ready now (customers/contacts split). Design then: pick survivor, re-point leads/proposals/links, keep audit trail, irreversible-with-confirm | v1.1 CRM polish |
