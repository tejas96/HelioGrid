...
**Status:** MIXED. Built and current: the identity spine, tenancy, the Temporal worker, the market
packs. Design intent with no code behind it: `voice`, `adapters`, realtime, storage, the studio
flow. Where a sentence here and the tree disagree, the tree and `architecture.md` win — fix the
sentence; do not build on it.
...
packages/
  ...
  adapters/   NOT YET CREATED — provider adapters land with the first module that needs a port (07)
  db/         Drizzle schema + migrations
  ui/         the design system, BOTH platforms — `<Name>.tsx` (web) + `<Name>.native.tsx` (RN)
  ...
  config/     shared tsconfig presets only — biome and dependency-cruiser config are root-owned
```
...
**Internal layout of a backend app** (binding closed set — the app's own `CLAUDE.md` carries the rules):
...
apps/{api,worker}/src/
  ...
  common/            common.module.ts · tokens.ts · request-id.ts · logging.ts ·
                     guards/ decorators/ filters/ interceptors/ errors/ db/ temporal/
...
- **Primary: app-layer scoping.** Every query path goes through the tenant-scoped
  repositories in `apps/api` (`modules/<m>/*.repository.ts`, the only files where `db` is legal —
  `apps/api/CLAUDE.md`); each runs inside `withTenantTransaction`, so there is no un-scoped query API to forget to scope.
...
- **Roles:** twelve stackable presets (F2-01; `packages/domain/src/authz/roles.ts`) —
  OR-across-roles, widest visibility wins per domain. The session projection carries the
  membership's roles; the guard asks `packages/domain` for the capability. The normative matrix
  is `F2` §F2.5; this doc fixes only the mechanism.
