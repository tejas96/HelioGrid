...
docs/engineering/07 §4 (v1 draft) and the BYO-porting/DTMF-send wording of BLUEPRINT directive 7 (see
...
(docs/engineering/07 §4 matrix) — it never throws, and it never silently no-ops. Example: outbound
...
ComplianceGate is unchanged and non-swappable (docs/engineering/07 §6); the control plane classifies
