...
Each row below is satisfied by its owning module's FIRST migration. Migration 0001 is the
market pack (`T-FCORE-016`); the identity/platform spine follows it, because `tenant` carries the
market foreign key and the pack carries none out.
