...
| notification_preference | V1 | 0 | A per-user push mute for one notification type-group. Push only — never the in-app record, never Owner billing/compliance events.  | tenant | user_ref; type_group; push_muted (boolean); natural key user × type_group | — | F6-15 |
