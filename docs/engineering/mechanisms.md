...
| M93 | A pull request is one complete task and at most 1,000 non-generated lines | `changes` job step in `ci.yml` | PARTIAL · red 2026-09-06 | The count is held: generated files (the lockfile, the emitted OpenAPI, the catalogs, theme output, snapshots) are excluded, and a `size-exception` label, which only a person can add, records a one-off in the PR. Completeness is review-only: `/ship` refuses to open a PR with an unproven done-when line, and nothing mechanical reads that table. |
