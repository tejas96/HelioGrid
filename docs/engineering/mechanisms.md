...
| M92 | An instruction file stays within its budget and names no gate outside this file | — | **NONE** | Closing it: three checks in `gates.py`, which already reads docs — line budgets, no dates in the constitution, and every enforcement phrase resolving to a row id here. |
