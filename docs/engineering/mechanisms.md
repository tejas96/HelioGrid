...
| M102 | A lapse forfeits price protection, and the grace window does not (`BM-42`, `Q43`) | unit `forfeitsPriceProtection` over `BILLING_STATES` · exhaustive `Record<BillingState, boolean>` | HELD · red 2026-09-07 | The `Record` makes a seventh state a compile error; the test catches a misclassified one. It cannot see `M12` billing a lapsed tenant at the old row anyway, nor the cancellation and dunning copy that `BM-42` requires say the forfeiture out loud — `M12-57` and review. |
...
| M71 | Coverage thresholds land with the slice | vitest `thresholds` | PARTIAL · red 2026-09-07 | Fifteen entries have a threshold, all at 100%, under `packages/domain/src/`: `calling/**`, `certification/**`, `format/**`, `market/**`, `money/**`, `pricing/**`, `rails/**`, `subsidy/**`, `tax/**`, and the six FILES `commerce/caps.ts`, `commerce/grandfathering.ts`, `commerce/meters.ts`, `commerce/soft-block.ts`, `commerce/tiers.ts` and `commerce/trial.ts`. Those six are the shape of a real limit: `all: true` scores a file with no runtime reader 0%, and `commerce/`'s remaining closed vocabularies — `states.ts`, `costs.ts` — have none until their consumers land, so a folder glob there would buy an import-only test and nothing else. Every other covered path has no threshold. |
