...
| M71 | Coverage thresholds land with the slice | vitest `thresholds` | PARTIAL · red 2026-09-07 | Twelve entries have a threshold, all at 100%, under `packages/domain/src/`: `calling/**`, `certification/**`, `format/**`, `market/**`, `money/**`, `pricing/**`, `rails/**`, `subsidy/**`, `tax/**`, and the three FILES `commerce/meters.ts`, `commerce/tiers.ts` and `commerce/trial.ts`. Those three are the shape of a real limit: `all: true` scores a file with no runtime reader 0%, and `commerce/`'s remaining closed vocabularies — `states.ts`, `costs.ts` — have none until their consumers land, so a folder glob there would buy an import-only test and nothing else. Every other covered path has no threshold. |
