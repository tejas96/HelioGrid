...
Extension point: one folder per module slice (authz/, auth/, branding/, tenancy/, calling/, certification/, commerce/, format/, market/, money/, pricing/, projects/, rails/, shell/, subsidy/, tax/ today); each
...
- UI composes from @heliogrid/ui ONLY (raw elements lint-banned across app/ and
  features/). app/ routes, a page in one session group ((door) (open) (inside)) whose layout gates it; features/ own capability; page.tsx never holds logic.
