...
| apps/web pages route, features own the capability | dependency-cruiser `web-app-imports-feature-barrel-only` (a page reaches a feature only through its `index.ts`) and `web-feature-no-cross-internals` — both live, both proven to fire on a probe. The 50-line page cap is Biome `noExcessiveLinesPerFunction` (maxLines 50) scoped to `apps/web/app/**/page.tsx` — **LIVE since 2026-07-31, and proven to fire on planted filler.** A lint rule rather than a line-counting script is a standing preference (owner directive 2026-07-31): it measures the function BODY, so imports and the doc comment do not eat the budget, and it reports at the exact line. **The cap is PER-FUNCTION**, so a page file that stays under it by splitting into several inline sub-components (each body under 50 lines) is not caught by this rule — dependency-cruiser `web-app-holds-no-components` is what stops that shape, by forbidding `app/` from importing any component, hook or style file from elsewhere in `app/`. ADR-0022. The previous ONE-FOLDER-PER-ROUTE convention had no size mechanism at all, and `app/login/page.tsx` reached 388 lines. | lint | `.dependency-cruiser.cjs`, `biome.json` (Task 6), `apps/web/CLAUDE.md` |
