...
| **No reinventing a `packages/ui` component (web)** | Biome `noRestrictedElements` on `app/**` + `features/**` (design-reference exempt): raw `button`/`input`/`a`/`select`/`textarea` rejected, each with the component to use instead. Added 2026-08-01 — the `hg-*` gate above only sees legacy class names, so a Tailwind-styled `<button>` passed adherence, cruiser AND typecheck | lint | `biome.json` |
| **No reinventing a `src/ui` component (RN)** | Biome `noRestrictedImports` on `screens/**`: `Text`/`TextInput`/`Button`/`Pressable`/`Touchable*`/`Switch` may not come from `react-native`. `View`/`ScrollView`/`StyleSheet`/`Platform` are layout and stay allowed. Law 7 — the web rule alone would cover half the product | lint | `biome.json` |
| Business enums come from `packages/contracts` | **Instruction + review only.** No honest gate: the 3 literal unions in `apps/**` are local UI state (`Step`, `OtpFailure`), not contract duplicates, so a blanket "no literal unions in apps" rule would be 100% false positives and get switched off. Precise detection needs value-set comparison against every `z.enum` — a checker script, which needs an owner ruling (§Process) | review | `apps/web/CLAUDE.md`, `epc-lens` |
