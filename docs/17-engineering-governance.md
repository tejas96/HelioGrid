...
| `tenant_id` never crosses the wire in a body or query | `tenant-id-in-body.ts` walks `apiContract`, unwraps Zod wrappers, and rejects `tenant_id`/`tenantId` at ANY nesting depth (objects and array members). Static — runs before the DATABASE_URL check, so it never skips. Job envelopes are out of scope and legitimately carry `tenantId` | invariant | partial — checks `body` and `query` only, NOT `pathParams`; and vacuous until a route declares a body or query schema, which it says when it runs | `tests/invariants/src/` |
...
|---|---|---|
| Commit, push and PR each need their own owner yes | An agent's own restraint — the harness cannot tell an approved commit from an unapproved one, and the failure mode is generalising ONE approval across a whole flow (observed 2026-08-03) | `CLAUDE.md` §8, `/finish` |
...
| Reference integrity (`docs/NN §M`, section citations, relative links) | Owner ruling 2026-07-30: greps, not a checker script. Three greps over `git ls-files`; two `docs/08 §…` citations inside sha256-locked migrations are unfixable and skipped by name | **PARTLY OWNED:** `arch-reviewer` check 8 sweeps dead paths left by deletions and moves. Section citations, `docs/NN §M` references and relative links are checked by nobody |
