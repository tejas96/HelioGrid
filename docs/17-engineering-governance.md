...
| Only apps read the environment | `@heliogrid/env` is the one package that touches a raw source. Turborepo tag `env`, allowlisted from the `app` tag ONLY — so `packages/domain` (ADR-0021: never an env read) and `packages/db` (its migrator takes the URL as a parameter) are structurally unable to import it. Both proven blocked 2026-07-30. Biome `noProcessEnv` is `error` repo-wide with `packages/env/src/**` the allowlist entry. | lint | `packages/env/`, `turbo.json`, `biome.json` |
