...
- **Spec**: `docs/product-journey.md` (master, D1–D39 locked decisions), phase-3…10 prompt packs, DESIGN-SYSTEM.md ("Instrument": graphite+brass, N1–N10 hard rules), 85 UX mockups in `/Users/devtejas/Downloads/HelioGrid UX/` (production specs, 375/1440 viewports, seeded states).
...
5. **Production launch target: next month** — roadmap defines an honest Launch-1 scope; mobile app may trail core web launch by weeks.
...
9. **The 3D Design Studio is THE flagship feature — nothing is compromised against it.** The phase-10 tool census (`explore/phases710.md` §2, screens 10.1–10.11) is the binding acceptance checklist for the studio port: **every tool and every computed output survives**, refactored to the design system, touch-first, with the scale program (blocks/zones, GPU shading, trackers, terrain) treated as investment INTO the studio moat — never as a reason to cut studio capability. Studio port is first-class in Launch-1, not trailing. Mobile presents the full-parity touch studio (responsive web at 375px per D2) through an authenticated seamless WebView; no studio feature is dropped on any surface.
...
- Fly Machines in `bom` (capacity-tight: web/api `min_machines_running=1`; `sin` overflow fallback). Process groups: web, api, worker (autostop=off, larger machines), voice, powersync.
- **DB: Fly postgres-flex (unmanaged) in bom — ⚠️ flagged risk: Fly has DEPRECATED unmanaged Postgres (self-support only; wal-g not bundled).** User chose Fly-native; mitigations are MANDATORY and in-scope for Launch-1: 3-node repmgr HA; **two backup layers → Tigris** (pgBackRest/Barman WAL archiving + nightly `pg_dump` logical dumps); restore drill before launch and monthly; disk/replication/OOM alerts; documented escape hatches (Fly MPG in `sin`, or external managed Mumbai Postgres) via logical replication — plain Postgres, nothing locks in. Revisit at scale.
...
### Auth
**Better Auth self-hosted** (organization + phoneNumber + jwt plugins) on our bom Postgres — phone PII stays in India. **MSG91** OTP (~₹0.15/SMS; WhatsApp-OTP fallback; DLT handled — registration lead time 1–2 weeks is a Launch-1 critical-path item). Web: cookie sessions. **Bare RN: framework-agnostic Better Auth client + custom storage adapter over react-native-keychain** (verified pattern; keychain tolerates Better Auth's colon-separated keys — cleaner than expo-secure-store; week-1 spike). JWT claims `{tenant_id, roles[]}` → Nest guards (primary authz; 6 stackable preset roles, OR-across-roles, widest visibility) + RLS backstop. Customer links: separate stateless signed tokens (HMAC, scope+expiry), never touch sessions.
...
Field-first app: My Day, leads, quick-add, surveys (offline), visits, notifications; studio opens as authenticated WebView (full parity per D2 is met by responsive web; native canvas editing is NOT rebuilt). Push: **Notifee + react-native-firebase** (FCM/APNs direct). i18n: Lingui metro transformer (verified RN≥0.73, no Expo needed). Tokens: react-native-keychain. CI: macOS lane for iOS builds. Ship: TestFlight + Play internal → stores (store-review lead time flagged in roadmap; mobile may trail web launch).

### Billing & entitlements (v1 — supersedes D38)
- **Platform SaaS billing: Razorpay Subscriptions** — UPI AutoPay primary (₹15k/debit cap fits tier prices) + card e-mandate fallback; **native trial support** → trial-only model (14–30d full-feature, no free tier); pre-debit notifications handled by Razorpay.
...
- Pricing tiers (designed in 01-business-model): org-level capacity-tiered, priced under ARKA/Reslink (e.g. Growth ~₹4–5k/mo, Pro ~₹8–10k/mo with voice bundle, Enterprise custom); voice minutes + AI detections metered beyond bundles.
...
### Design system & i18n
`packages/tokens`: Style Dictionary source → CSS vars (web, Tailwind v4 full import — no legacy layer needed in fresh repo) + RN theme object; verified contrast ratios as metadata; N1–N10 + touch contract + DoD as `design-spec` law. **Lingui v5** one catalog for Next.js + RN; per-user language re-render; Devanagari (Noto Sans Devanagari chain, explicit RN fallback handling); ₹ Indian grouping everywhere.
...
docs/
  ...
  14-build-roadmap.md (Launch-1 scope + module order + forward-compat register)
...
9. 14-build-roadmap: Launch-1 (month 1) = **studio web port as flagship (tool-census acceptance gate)** + auth/tenancy/billing foundation + CRM core + proposal builder + customer link + remote survey; Launch-2 (+weeks) = RN field app, physical-survey offline, voice agent GA (DLT/number-provisioning/onboarding lead times on critical path), dashboards; then scale program phases (blocks/zones → GPU shading → trackers/terrain) as studio-moat investment. Week-1 spikes listed: Better Auth phone-OTP on bare RN, pgBackRest→Tigris archive+restore drill, ts-rest 3.53/Zod-4 status check, Tigris single-region pin via `fly storage create`, Exotel BYO-number porting mechanics + AgentStream DTMF.
