...
9. 14-build-roadmap (as amended by directive 10): Launch-1 (month 1, web, no studio/no offline) = auth/tenancy/billing foundation + CRM core + catalog + **Path B proposal builder** + customer link + remote survey (thin domain subset: geo/roof-factory/roof-ai/finance kernels) + projects + dashboards-lite; Launch-2 (+weeks) = RN field app (online-first, repository abstraction), physical survey (online), voice agent GA (DLT/number-provisioning lead times on critical path), notifications/search, full dashboards; **final phases = 3a studio port/enhance (tool-census acceptance gate, Path A + BOM activate) then 3b offline (PowerSync)**; then scale program phases (blocks/zones → GPU shading → trackers/terrain) as studio-moat investment. Week-1 spikes: Better Auth phone-OTP on bare RN, pgBackRest→Tigris archive+restore drill, ts-rest 3.53/Zod-4 status check, Tigris single-region pin via `fly storage create`, Exotel BYO-number porting mechanics + AgentStream DTMF (PowerSync spike moves to phase 3b).
