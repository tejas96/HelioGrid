...
`docs/prd/` becomes the single canonical product specification, detailed enough that UX, PM,
...
| # | Decision | Ruling |
|---|----------|--------|
| DD1 | PRD structure | **Modular suite** under `docs/prd/`: overview + personas + journey map + business model + 8 foundation specs + 13 module PRDs + 4 registers (§4) |
...
| DD14 | Repo handling | Write everything under `docs/prd/`; **no git init, no commits** (owner instruction 2026-08-03). Source `docs/` and `design/` untouched forever |

## 3. Source corpus and reading rules

**Corpus (all of it is read):** `docs/00`–`16` (19 documents), `docs/prd/_process/product-journey.md`
(master spec: D1–D39 census, 9-stage journey, customer journey C1–C13, roles matrix),
`docs/prd/modules/M05-studio/studio-census.md` (binding studio acceptance gate), `docs/engineering/architecture.md`,
`docs/engineering/forward-compat.md`, `design/ds-source/` (design system). ~85,000 words.
