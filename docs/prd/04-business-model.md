...
`foundations/F4-data-integrity.md` (`F4-21`, the field-photo carve-out),
...
- Given any two tiers, when their capability sets are compared, then they are identical and only
  ceilings/counts/bundles/storage differ (BM-05, BM-12) — service terms may also differ per tier,
  but as pricing-page copy (BM-14), never as a BM-12 axis or a book field (BM-41).
...
- Given photographs already captured on a field device and a tenant in any blocked state, when
  the device reconnects, then those photographs upload and read; and given new field capture
  during dunning, when any enforcement design addresses it, then capture continues through the
  full `past_due` grace window and pauses only at `halted`, with a halt that lands mid-visit
  letting the current visit complete (BM-36; mechanics M12-27).
...
and the owner's 2026-08-04 ruling **re-validated them for V2's bigger box: base tiers stand
...
(BM-17/BM-26 — verification is the revisit trigger).
...
- *A book add-on value is still draft (the V2 add-ons)* → the meter exists (BM-21/BM-22) and the
  IN book carries the owner's draft numbers (owner ruling 2026-08-04), but the meter
...
- Given the two V2 add-on slots, when the IN book is read today, then each slot carries the
  ...
  (BM-41, `F1-61`; owner ruling 2026-08-04).
...
- *A tenant churns (`cancelled`/`halted`) and reactivates — inside or after the horizon* →
  ...
  lapsed subscription (owner ruling 2026-08-04; mechanics M12, `M12-57`).
...
the retired traceability register.*
