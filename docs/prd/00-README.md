...
| `docs/tasks/` | **392 engineering tasks** in 19 files. Each names its requirement rows, its screen (`DESIGN: SCR-… → PENDING` until the screen is approved), its acceptance criteria copied verbatim from the PRD, and — for studio tasks — the POC files to port and the defects to fix. Every row lands in exactly one task or is explicitly a law enforced elsewhere. `docs/tasks/README.md` states the rules. | Engineers, QA |
