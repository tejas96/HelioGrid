...
(`M03-18`, `BRIEF`); and **four `REC` rows** (`M03-16`, `M03-17`, `M03-37`, `M03-43`) carry the
module's recommendations ·
...
`modules/M07-sales-execution.md`, `docs/prd/owner-brief-2026-08-03.md` §Marketing,
*retired: PRD design note* §2 (DD2, DD4, DD5) and §11
...
(`docs/prd/owner-brief-2026-08-03.md` §Marketing):
...
recorded in `M03-01` and `M03-02` — **recorded, never silently resolved** (design spec
...
**Supersession 1 — lead-capture channels (`M03-01`).** `D13` fixes the v1
...
**Supersession 2 — outbound sending (`M03-02`).** `D32` rules that there is
...
- **The transactional lane (owned product-wide, not by this module's campaign machinery).** The
  ...
  class, with composed copy-paste as the no-channel fallback** (owner ruling 2026-08-04; `M03-03` states the two-lane law). This module supplies the connection those
...
labelled as recommendations. It is not a market-specific
...
|---|---|---|---|
| M03-01 | **This module is the superseding specification for `D13`'s deferred capture channels.** Website forms, inbound business-messaging, social lead forms, email replies and SMS replies are lead-capture channels of this product, specified in §M03.3 and §M03.4. `D13` is not overturned as a reading of v1 — it continues to govern `modules/M02`, which specifies none of these channels (`M02-17`) — it is superseded **as scope**, by the brief, under design spec §2 `DD2`. This row is the record of that contradiction. | `BRIEF` — `docs/prd/owner-brief-2026-08-03.md` §Marketing ("capture leads … across channels such as Email, WhatsApp, Facebook, Instagram, SMS"); authority *retired: PRD design note* §2 `DD2` + §11; supersedes `SRC` `D13` (v1 lead sources; docs/15 HONORED — "later" cards, `UXG-03`) | P0 |
| M03-02 | **This module is the superseding specification for `D32`'s no-sending rule, in the campaign lane only.** The product sends marketing messages on the tenant's behalf, from channel identities the tenant owns (§M03.3), metered per send (§M03.6). `D32` is superseded **as scope for campaigns**, by the brief, under `DD2`; this row is the record of that contradiction. | `BRIEF` — `docs/prd/owner-brief-2026-08-03.md` §Marketing ("engagement across channels…", "manage marketing campaigns"); authority *retired: PRD design note* §2 `DD2` + §11; supersedes `SRC` `D32` (no business-messaging integration in v1; docs/15 HONORED) | P0 |
| M03-03 | **Two lanes, both real (owner ruling 2026-08-04).** The **campaign lane** is unchanged: this module's promotional sending, metered and consent-gated as specified here. The **transactional lane now exists product-wide**: the one-to-one customer moments the v1 source held composed-not-sent — `modules/M06`'s proposal share, `modules/M02`'s message action (`M02-33`), visit confirmation (`M02-47`) and no-show reminder (`M02-48`), `modules/M08`'s handover pack, and `foundations/F5`'s customer moments (`F5-14`, `F5-16`, `F5-48`) — **send automatically from the tenant's connected official channel** (the same connection §M03.3 establishes), under the **utility/transactional template class the pack's rules define**, distinct from the marketing lane's consent and metering; **copy-paste composition is the fallback** wherever no channel is connected, and on that path no delivery is claimed. `D32`'s manual-only rule is fully retired. The campaign lane never carries a transactional moment, and a transactional send never burns the marketing meter. | `BRIEF` — the boundary this module's supersession implies; transactional lane per owner ruling 2026-08-04, superseding `SRC` `D32`'s manual rule; template-class per pack rules (`F1-15`/`F1-38` consumed) | P0 |
| M03-04 | **Because this module controls the sending, it reports send state — and only what the channel actually tells it.** A campaign send carries a per-recipient state drawn from the channel's own reporting: queued · sent · delivered · failed (with the channel's reason) · opted-out-before-send. Where a channel does not report a state, the product shows **"not reported by this channel"** and never a zero, a blank that reads as success, or an inferred delivery. Opens and clicks are the link's own events, never a delivery claim (`F5-28`, `F5-29` consumed — the customer-link surface has no delivered state and this module does not give it one). | `BRIEF` — the delivery-state semantics the outbound-sending supersession (`M03-02`) assigns to this module · honesty obligations consumed from `F8-34` (honest state), `F8-36` (honest failure), `F8-12`; channel-capability rule at `M03-26` | P0 |
| M03-05 | **No AI feature is invented here.** The brief's instruction is carried as a law of this module: *"Do not invent AI features beyond what is supported or clearly proposed; identify additional ideas separately as recommendations."* The voice-AI follow-up capability the brief preserves is `modules/M07`'s and is `SRC` (`M03-25`). Any marketing-side automation or generation beyond the brief's sentence appears **only** as a `REC` row (`M03-16`, `M03-17`, `M03-37`, `M03-43`), tagged as a recommendation — never as core scope, never phrased as though the corpus or the brief asked for it. | `BRIEF` — `docs/prd/owner-brief-2026-08-03.md` §Marketing (quoted); tagging law `00-README.md` §Tag vocabulary + design spec §5, §11 ("marketing-side AI beyond source/brief is `REC`-only") | P0 |
...
- *A reader cites this module to justify sending a proposal automatically* → now correct in the
  ...
  transactional template class (`M03-03`, owner ruling 2026-08-04; `modules/M06` owns the
...
- *Someone asks for "AI subject lines" in the next release* → it exists here already, as
  `M03-43`, tagged `REC` with its rationale — which is the whole point of the tag.

**Acceptance criteria.**

- Given either recorded contradiction (`D13` vs the brief's capture channels, `D32` vs its campaign
  sending), when a reader looks for its owner, then this document states the supersession explicitly, names
...
- Given any transactional surface listed in `M03-03`, when it produces a customer message with a
  ...
  it composes for copy-paste and claims no delivery (`M03-03`, owner ruling 2026-08-04).
...
- Given any requirement in this module that proposes generation or automation beyond the brief's
  sentence, when its tag is read, then it is `REC` with its rationale in the row
  (`M03-05`).
...
| M03-16 | **Multi-step nurture sequences — recommended enhancement, not v1 scope.** A campaign in this release is one audience, one content, one send moment (`M03-12`). A sequence — several steps with waits and branch-on-response between them — is the obvious next ask and is **recommended**, not specified: it would need its own suppression, consent-recheck and metering semantics at every step, and the brief asks for campaigns rather than journeys. | `REC` — this suite's recommendation; explicitly **not** in `docs/prd/owner-brief-2026-08-03.md` §Marketing (which names campaigns, capture and pipeline hand-off) | P2 |
| M03-17 | **Split (A/B) testing of campaign content — recommended enhancement, not v1 scope.** Sending two content variants to slices of one audience and reporting the difference is standard in the category and is **recommended**; it is not in the brief, and it interacts with this module's correlation-only reporting law (`M03-53`) — a variant comparison is still correlation, and would need to say so. | `REC` — this suite's recommendation; not in the brief; interacts with `F8-30` consumed at `M03-53` | P2 |
...
owner's 2026-08-04 ruling it resolves over the **whole lead base for any holder of the
...
aggregate-only — segments and counts, no record reads (owner ruling 2026-08-04);
...
where the tenant has authored no version in that language, `F3`'s conservative rule applies — the
original language shown with a small note saying so — and the gap is surfaced to the author
rather than machine-filled.
...
- *A social lead form returns an email and no phone* → cannot arrive from a connected form: every
  connectable form requires the phone field, and one without it is refused at connection (`M03-33`).
...
- *A visitor's language has no authored form version* → the gap is surfaced to the tenant author;
  nothing is machine-translated (`F3-10`).
...
| M03-43 | **AI-assisted campaign content drafting — recommended enhancement, not v1 scope.** Generating subject lines, message copy or variant suggestions from a prompt is the category's most common AI feature and is **recommended**, not specified: the brief forbids inventing AI beyond what is supported or clearly proposed (`M03-05`), and any such feature would additionally sit under the market's tiered AI-disclosure law (`F1-36`(d)). | `REC` — this suite's recommendation; explicitly outside `docs/prd/owner-brief-2026-08-03.md` §Marketing's no-invented-AI instruction; the tiered AI-disclosure law cited as an input, not restated | P2 |

**Recorded, not resolved — `M03-40`'s description of `M01-55` predates the transactional-lane ruling.** The cell above reads
`M01-55` as establishing message templates "for the moments the product **composes** text a person
sends." `M01-55` no longer says that: since the owner ruling of 2026-08-04 the composed
...
specifies remain campaign-lane objects, which that ruling left unchanged. **The requirement cell is not
rewritten here.** The divergence is recorded at this line for the PRD owner, and the verbatim copy
at `docs/tasks/M03-marketing.md` (T-M03-009) carries the same note so the build reads the lane
from `M03-03`.
...
- *A recipient's language has no authored version* → the gap is surfaced to the author; nothing is
  machine-translated or substituted (`M03-38`).
...
| M03-52 | **A meter whose book value is not yet sellable cannot be sold, and the module says so rather than defaulting.** `BM-21`'s per-channel bundles and overage rates now carry the owner's **draft** values in the India book (owner ruling 2026-08-04: Starter 500 / Growth 2,000 / Pro 10,000 sends/mo; overage ≈ WhatsApp ₹1.5 · SMS ₹0.35 · email ₹0.10) — **draft pending rate-card verification** (`BM-17`/`BM-26`), and `04-business-model.md` fixes the behaviour: a draft rate is never silently treated as launch-final, and the meter is not sellable until its rate card verifies. This module surfaces that state honestly on the channel-health surface: the channel connects, and metered selling in that market waits on the verification. | `BRIEF` — carried consequence; `BM-21` / `BM-41` / `F1-61` and the draft-pending rule consumed from `04-business-model.md`; draft values per owner ruling 2026-08-04 | P1 |
...
- *The market book's bundle rate is still draft* → the channel connects and metered selling waits
  on rate-card verification of the draft values (owner ruling 2026-08-04); the product does
...
- **The superseding specifications** for `D13`'s capture channels and `D32`'s campaign sending
  (`M03-01`, `M03-02`) and the lane boundary that keeps them contained (`M03-03`).
...
| `foundations/F5-customer-link.md` | The no-PII-in-URLs and no-third-party-script laws that campaign links and landing pages obey (`F5-77`, `F5-29`); the branding/white-label routing (`F5-81`–`F5-83`); and the transactional send-channel ruling, which this module consumes and does not own. |
...
| `04-business-model.md` | The marketing-sends meter and its channel boundary (`BM-21`), the canonical meter set (`BM-16`), the cap law with its 80% pre-warning (`BM-34`), the soft-block law and matrix (`BM-32`, `BM-35`), the billing-state names (`BM-33`), and the empty-book-slot rule (`BM-41`). |
...
- **No transactional sending through campaign machinery.** The transactional lane is real and
  product-wide (owner ruling 2026-08-04 — automatic sends from the connected channel,
...
- **No invented AI.** The brief's own instruction (`M03-05`); generative and automation ideas are
  `REC` rows (`M03-16`, `M03-17`, `M03-37`, `M03-43`), never core
...
- **No delivery claim a channel did not make.** Where a channel reports nothing, the product says
  ...
  `F5-28` as amended by owner ruling 2026-08-04).
