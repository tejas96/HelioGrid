...
system"). Its third choice reads *Keep both leads* at every entry point, because that is true whether the
sheet opens on a capture or on an existing record (owner ruling 2026-09-24). Choosing **Open existing** discards the in-progress capture after confirming and opens
...
- Given an assign or a mark-junk, when the tap is made, then the row shows the action in progress
  ...
  with the reason named (M02-67, `F8-36`, owner ruling 2026-08-15).
...
- Given the message action with a connected transactional channel, when it is used, then the
  ...
  and no delivery state is claimed (M02-33, owner ruling 2026-08-04).
...
| ID | Requirement | Tag + source pointer | Tier |
|---|---|---|---|
| M02-39 | **Qualification captures six things, inline on the lead, never as a separate form: monthly bill, roof ownership (own or rent), roof type, obvious shading, timeline, and decision maker.** These are the six facts that decide whether a lead is real, and they are answered on the call, in the lead, as the conversation happens. **Option sets (owner ruling 2026-09-24):** roof type — RCC flat · Metal shed · Tile · Ground array (`M05-25`'s census set); shading — None · Partial · Heavy; timeline — Within a month · 1–3 months · 3–6 months · Later; decision maker — This person · Someone else. | `SRC` — `S3.screen.3` ("six things that decide whether this is real… Inline, not a separate form"); `S3.happy` ("rep calls within the hour → qualifies on the call"); `DOC04.qualification` (the same six as a form snapshot) | P0 |
...
- *No-show* (`S3.wrong.7`) → reschedule in one action; exactly one reminder, sent via the
  ...
  (M02-48, owner ruling 2026-08-04).
...
- Given a completed booking with a connected transactional channel, when the confirmation is
  ...
  no delivery (M02-47, owner ruling 2026-08-04).
...
- Given a lead marked lost with reason postponed, when the given date arrives, then it resurfaces
  ...
  Lost reason per owner ruling 2026-08-04 — then no call task is raised for six months
  (M02-54).
...
and the contacts; every contact of both records survives, and only the primary is chosen. One number is
one contact (M02-02): a contact whose number the survivor already holds joins that contact and keeps
its role label. A named
...
- **No marketing automation in this module** (`S2.notv1.2`). Campaign management is the V2
  ...
  drips. The v1-source non-goal and the brief's mandate are reconciled by design spec §2 `DD2`'s
  brief-driven supersession, with `modules/M03` owning the superseding specification (`M03-01`).
...
- **No delivery state on the copy-paste fallback path** (M02-33): where no transactional
  ...
  message arrived. *(Superseded in part by owner ruling 2026-08-04: with a connected
  official channel, transactional messages — visit confirmations, reminders, links — send
  automatically and carry the channel's honest delivery states (`M03-03`). The campaign lane
  stays `modules/M03`'s.)*
