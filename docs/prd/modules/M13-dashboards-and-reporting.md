...
| M13-34 | **Project Manager — home: their projects ordered by days-in-stage, oldest first** — a blocker flags a card and never reorders it (owner ruling 2026-09-24) — each card: customer, size, value, days in current stage, payment collected vs due, blocker flag with who is being waited on (card facts `M08`'s; ageing per M13-25). | `BRIEF` — `PS-21` (home for a V2 persona); card contents from `SRC` `S8.screen.1` consumed via M08 | P0 |
