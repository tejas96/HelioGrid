...
| MS4-06 | All three entry paths are REAL on every picker: **Browse** the resolved catalog · **Upload datasheet** (PDF extraction with review-before-commit per M01-40) · **Enter specs manually** — empty with nothing selected, pre-filled from the selected item per M01-39 (certifications and price never copied; the save is a new own SKU; owner ruling 2026-09-07) — plus in-flow **Excel import** (M01-41). The POC's non-interactive caption (`.23/.24`) is retired. | `BRIEF` S4-1 | P0 |
| MS4-07 | Provenance is visible per entry — the catalog's data-provenance label unchanged (M01-35: verified-datasheet / tenant-provided / representative), with effective-from where present (the date of M01-44's rate entry; an entry with no rate carries no date) — surfaced in the picker row and carried into compare and BOM (S4-1 activates `.72/.73`; F8). Which tier the resolved row came from (tenant override / tenant item / platform item / custom) is the separate axis M06-31 carries (owner ruling 2026-09-07). | `BRIEF` S4-1 | P0 |
...
Edge cases: a capacity above the sanctioned load raises M05-20's informational warning here (a provisional cue on DC kWp, unit stated, until an inverter is chosen) and in the inverter section (inverter AC kW × count against the sanctioned load in kW) — the kW limit named, "verify with the DISCOM", Continue unaffected; silent when the sanctioned load is blank or 0 or the site is open-access.

### MS4.5 — Inverter, topology & MLPE

| ID | Requirement | Tag | Tier |
|---|---|---|---|
| MS4-18 | Recommendation engine (one inverter model per design — every candidate is one unit × count, M05-40): scans the resolved catalog for units matching the site's phase, sizes count ≤4, keeps only candidates inside the DC/AC eligibility band (0.90–1.35), then picks the candidate whose ratio is CLOSEST to the 1.15 target (minimises \|ratio − 1.15\|), tie-breaking on lower total price among candidates with a tenant rate — an unpriced candidate orders after every priced one, then by fewer units, then lower AC kW, then catalog order (owner ruling 2026-09-07); the banner states unit, count, phases and resulting ratio; never auto-applies (`.34–.36`). | `SRC-CODE` | P0 |
...
| MS4-21 | Inverter count input — the count of the ONE selected unit (M05-40; owner ruling 2026-09-07) — enforces its stated maximum as well as its minimum (S4-4.6 fixes `.39`); the count propagates to ratio, BOM and stringing (`.39`). | `SRC-CODE` + `BRIEF` S4-4.6 | P0 |
...
| MS4-23 | Inverter spec contract: AC kW, phases, MPPT windows (count/min-max V/current/strings), max DC V, efficiency, warranty — the fields electrical sizing depends on (`.78/.79`); a rate is not a spec — it is the tenant's, resolved at BOM time (M01-37, M01-44, MS10-26; owner ruling 2026-09-07). | `SRC-CODE` | P0 |
...
| MS4-29 | Candidate set: certification-first shortlist ranked by cost-per-watt (₹/Wp on module nameplate DC) among candidates with a tenant rate, capped — unpriced candidates order after priced ones, by fewer units, then lower AC kW, then catalog order (owner ruling 2026-09-07) — with the CURRENT selection always included even when it wouldn't rank (`.50`). | `SRC-CODE` | P0 |
| MS4-30 | The 11 comparison columns, each computed per candidate: option (with RECOMMENDED/CURRENT badges) · inverter (+count, DC/AC) · fits (panels × watt, achieved kWp) · module efficiency (STC area basis) · annual kWh (unshaded basis, stated) · net cost (with subsidy sub-line via pack) · payback (escalation + degradation model) · 25-yr savings (+ ROI %) · warranty · install complexity band (+ array weight) · action (`.51–.61`). A candidate whose component has no tenant rate carries no money columns — net cost, payback and 25-yr savings/ROI are absent, shown as missing per F8-01, never ₹0 (owner ruling 2026-09-07). | `SRC-CODE` | P0 |
| MS4-31 | Action states are honest: infeasible options cannot be applied (marked "check"), feasible ones offer Apply; the note taxonomy explains every zero/warn case verbatim (`.62/.63`) and carries 'no rate on file' for a candidate whose component has no tenant rate (owner ruling 2026-09-07). | `SRC-CODE` | P0 |
| MS4-32 | Matrix recommendation rule (feasible AND within the clipping limit) is stated on-screen and distinct from the picker's rule (`.64`), and states its fallback — candidates with no tenant rate rank after priced ones and carry no money columns (owner ruling 2026-09-07); decision cards below the matrix state topic, choice, reason and inputs (`.65`); the fixed-assumptions footnote lists escalation, horizon, margin and subsidy basis (`.66`). | `SRC-CODE` | P0 |
...
**Later.** Mixed inverter models in one design (a list with + Add inverter, summing kW AC): later; V1 is one model × count (M05-40; owner ruling 2026-09-07).
...
- Given a tenant with own SKUs and price overrides, When any picker opens, Then it lists the RESOLVED catalog — identical to what compare and BOM use (MS4-05). Given a missing product, Then browse/datasheet-upload/manual all work in-flow, with datasheet extraction reviewed before commit; given a selected component, Then Enter specs manually opens pre-filled with its specs and blank certifications and price, and the save is a new own SKU (MS4-06). Given a tenant-entered SKU, Then its provenance shows on the row and travels to compare and BOM (MS4-07). Given an on-order unit, Then the availability badge shows in the picker, not only in compare (MS4-08). Given an IN-market tenant, Then ALMM/DCR flags show and filter as pack-driven data (MS4-09).
- Given an emptied Max-watt filter, Then the list is not emptied and the bound reads as "no maximum" (MS4-10); given zero matches, Then the state explains and offers Clear filters (MS4-11). Given 24 placed modules, When a different panel is chosen from the picker, Then the guard dialog appears with the count and keep-or-refill choice, and one undo reverts everything (MS4-12). Given a catalog entry violating the spec gate, Then it never reaches the picker; given a module with no tenant rate, Then it is still pickable (MS4-13).
