...
- Given the owner edits the opening line, when they save, then the wording change takes effect
  ...
  (M07-10, owner ruling 2026-08-04).
...
| M07-24 | **Every call opens naturally — no proactive AI mention at IN launch — and "talk to a person" works at any moment (owner ruling 2026-08-04, replacing the former ≤30 s disclosure default).** The tiered law of F1-36(d) governs: the four hard floors always hold — the agent **never claims to be human**, **never denies being AI when asked** (honest answer plus an immediate human offer), hands to a human instantly on request, and every call is fully transcribed to the timeline; proactive disclosure is pack data (IN OFF until the TRAI identification rule binds, then auto-ON with owner notification; EU-class packs ON). Each call records what its opener played and how any are-you-an-AI question was answered (§M07.7). | `SRC` — `S7.rule.disclosure` and `DOC00.voice-touchpoint` as superseded by the tiered law — owner ruling 2026-08-04; `DOC04.call-ledger` (opener/answers recorded); `F1-36`(d) consumed | P0 |
...
- Given any outbound agent call, when it connects, then it opens with the configured natural
  ...
  (M07-24, owner ruling 2026-08-04).
...
| M07-30 | **A refusal is visible, never silent.** A queue entry the gate blocks persists its pre-dial verdict — registry-listed · no consent · do-not-call · outside window · quiet flag — on the queue and on the lead. A registry-listed customer is not dialed by the agent and **the rep is told to call manually**. **Manual dials (owner ruling 2026-08-04):** on an outside-window or registry-listed manual call action the product shows the verdict as a **warning-then-proceed** — the rep may proceed, and where they cite a customer request the "customer requested" context is logged with the call; the statutory gate enforcement itself binds automated dials (the three lanes, F1-36(b)). | `SRC` — `S7.wrong.1` (post-overlay: gate enforcement, not a default); `DOC04.call-queue-compliance` (persisted verdict vocabulary); manual-dial warning-then-proceed per owner ruling 2026-08-04 | P0 |
...
- *Customer is on the do-not-disturb registry* (`S7.wrong.1`) → agent will not dial; the rep
  ...
  with any "customer requested" context logged — ruled 2026-08-04; the enforced lanes
...
- Given any connected agent call, when it completes, then the record shows the opener version
  ...
  proactive line played (M07-32, owner ruling 2026-08-04).
...
own entries (`F2.M07.control-agent-queue`, widened per owner ruling 2026-08-04); handing
...
- Given the owner removes any queued call — or the queuing rep cancels an entry they queued —
  ...
  timeline with its actor (M07-35, owner ruling 2026-08-04).
...
- Given a rep marks a lead lost, when they must pick a reason, then the surface offers the
  ...
  refuses a reasonless save, and postponed losses carry a date (M07-63).
