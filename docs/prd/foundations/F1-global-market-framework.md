...
- *A ruleset item's floor/default classification is unclear in a market's law* → the pack
  ...
  raised to the owner (F1-36's AI-disclosure item was exactly this case, resolved
...
- *A pack's declared scheduled-send hour falls outside that market's own messaging window* →
  ...
  ruling 2026-08-06). Both sides of that comparison are read on the **tenant's** timezone —
  the slot and the window alike (F1-10, owner ruling 2026-08-06): one clock, so "outside
  lawful hours" and "the last lawful moment before it" have a single frame. Past the close that
  moment is the close; before the open it is the **previous day's close**, and the resolution
  carries the day it lands on — a clock time alone would read as today's close, after the slot
  (owner ruling 2026-09-07). Only a pack revision that moves the floor reaches that near side: a
  tenant narrowing that would leave the send hour outside the window in force is refused at save
  (M01-56), and an early-evening slot never precedes a statutory morning open. *(Edge case added by
  the pass applying the send-hour ruling; before it no send hour was declared and this list had
  no case for one. Clock sentence added by the pass applying the window's-clock ruling, which
  supplies the frame the case had been stated without.)*
- *A tenant serves a customer in a different timezone from its own* → the send hour resolves on
  the **tenant's** timezone (F1-10, F1-15, owner ruling 2026-08-06), so the message goes at
  the EPC's evening hour and not the customer's — and the lawful-window check the hour yields to
  resolves on that same tenant clock (owner ruling 2026-08-06), so the recipient's own
  ...
  defect to be worked around per tenant. *(Edge case added by the pass applying the hour's-clock ruling;
  before it the clock was undecided and this list had no case for the two diverging. Extended by
  the pass applying the window's-clock ruling to name the window alongside the hour.)*
- *A tenant's own timezone differs from its market's default timezone* (F1-21 supplies a default
  ...
  not the market's (F1-10, F1-15, owner ruling 2026-08-06). This is the second face of the
  ...
  applying the window's-clock ruling; before it this divergence was an open question and no case
  could state an outcome.)*
...
- Given a pack-declared scheduled-send hour for an automatic transactional message, when the
  ...
  evaluated on that same tenant timezone (F1-10, owner ruling 2026-08-06), including where
  ...
  rulings 2026-08-06 on the send hour, its clock and the window's clock). *(Criterion added by the
  pass applying the send-hour ruling; before it no scheduled-send hour was pack data and no
  criterion here tested one. Amended by the pass applying the hour's-clock ruling: it previously
  read "then it goes at that hour in the customer's market timezone", a divergence against F1-10.
  Amended again by the pass applying the window's-clock ruling: the window clause previously named
  no clock for the window and now names the tenant's — one clock for hour and window.)*
- Given a statutory messaging window in force and a scheduled send hour that falls before it
  opens — reached only where a pack revision moves the floor — when the send is resolved, then it
  goes at the previous day's close and the resolution carries the day it lands on, so it never
  reads as today's close; and given the same hour falling after the close, then it goes at that
  day's close — one rule on both sides of the window, the message always earlier than its slot
  and never later (F1-15, owner ruling 2026-09-07).
...
than risking it (docs/engineering/07 §ComplianceGate, M07). What stays tenant-editable in IN, per D36:
...
law no tenant, preset or template edit can weaken (owner rulings 2026-08-04). The
...
| F1-48 | **Dates and calendar:** date style "12 Mar 2026"; default tenant timezone **Asia/Kolkata** (a default per F1-10); **the IN holiday calendar is authored EMPTY (owner ruling 2026-09-07)** — TCCCPR states no holiday rule and no row of this suite names a date, so the pack declares an empty calendar rather than a platform-authored list a tenant could never remove (F1-17 permits only narrowing). Every IN holiday is the tenant's own working calendar (M01-59), which the calling window (F1-36) and schedule defaults honor. *(Amended to that ruling. The row previously read "the **IN holiday calendar** is the pack calendar the calling window (F1-36) and schedule defaults honor" while naming no date anywhere in the suite — a statutory floor with no content — and the clause consuming it at F1-36b cited F1-50, which is metric units. The empty declaration is an authored value in the F1-62a sense, not a missing one, so nothing downstream waits on it.)* | `SRC` — `DOC10.dates-tz` (cited — F3); the empty calendar per owner ruling 2026-09-07, on the `F1-62`a authored-empty pattern, with the tenant calendar at `M01-59` | P0 |
| F1-49 | **Phone specification: +91**, E.164 identity; the OTP-destination allowlist is **+91 by default**, with other country codes enabled per market rollout — a switch, not a code change. | `SRC` — `DOC08.otp-destination` (docs/engineering/08); E.164 identity per `DOC04.user-lifecycle` (cited — M01) | P0 |
...
| ID | Requirement | Tag + source pointer | Tier |
|---|---|---|---|
| F1-54 | **IN determination (DPDP Act / DPDP Rules 2025):** the platform is **Data Fiduciary** for tenant users' PII and **Data Processor** for the EPC's customer data — the tenant is fiduciary for their customers; DPA terms ride in the subscription agreement. This is the IN instance of F1-23; any new market authors its own before tenants exist there (F1-05). | `SRC` — `DOC08.dpdp-roles` (docs/engineering/08 §9) | P0 |
| F1-55 | **IN residency:** all PII/primary data resides in India; payment instruments never touch the platform (F1-43); cross-border object storage is permitted under DPDP's **negative-list transfer model** — with a documented migration path if the list changes. | `SRC` — `DOC08.residency`; `DOC03.dpdp-residency` (docs/engineering/03 compliance ruling — same determination) | P0 |
| F1-56 | **IN rights map — access/export and correction:** tenant-level read + export always work regardless of billing state (the F1-24 law, IN instance); individual data-principal export is a support-backed workflow — JSON/CSV of all rows keyed to the principal, **30-day SLA**; correction happens in-app through the ordinary record edit. | `SRC` — `DOC08.data-rights-export`; `DOC08.data-rights-correction` (docs/engineering/08) | P0 |
| F1-57 | **IN erasure = anonymisation, never row deletion:** PII fields are overwritten — name becomes "Erased", phone becomes a keyed hash that preserves dedupe integrity — while financial/tax records (proposals, invoices, payments) are retained for the statutory period (F1-32). User accounts follow deactivate-never-delete with the same anonymisation. | `SRC` — `DOC08.erasure-anonymise` (docs/engineering/08); deactivate-never-delete per `DOC08.deactivate-never-delete` (dispositioned by Task 5, `F2-20`) | P0 |
| F1-58 | **IN consent records:** per-customer voice-call consent, recording consent, and DND/do-not-call flags — each with timestamps and source — are stored, surfaced pre-dial (the gate read, M07), and **exported on request**. | `SRC` — `DOC08.consent-records` (docs/engineering/08; pre-dial surfacing is M07's) | P0 |
| F1-59 | **IN breach duty:** notify the Data Protection Board and affected data principals; a grievance contact is published in-app. | `SRC` — `DOC08.breach-grievance` (docs/engineering/08) | P0 |

#### IN · pack.price-book

| ID | Requirement | Tag + source pointer | Tier |
|---|---|---|---|
| F1-60 | **The IN book is the source-derived first book**, in INR, ex-GST, canonical in `04-business-model.md` (one definition per fact): tier anchors ₹1,999 / ₹3,999 / ₹9,999 monthly + Enterprise custom, each with a yearly variant, and the v1 caps, bundles and overage rates as `04` defines them; benchmarked **under Reslink and ARKA at equivalent capacity**. This key records the book's existence, currency, tax posture and benchmark basis; no number is defined twice. | `SRC` — *retired: PRD design note* §2 DD6 ("India book (₹1,999/₹3,999/₹9,999/Enterprise + v1 caps/bundles) is the source-derived first book"); `DOC01.tier-table`, `DOC01.price-under-incumbents`, `DOC16.pricing-single-source` (all cited — `04-business-model.md` owns the numbers) | P0 |
| F1-61 | **V2 metered add-ons are priced in the IN book:** the per-tracked-seat field-workforce price (DD7) and per-channel marketing-send bundles (design spec §8) are IN-book rows. **Owner ruling 2026-08-04:** the rows now carry the owner's **draft** values — tracked seat ≈₹99/seat/mo beyond the tier's included allowance (Starter 0 / Growth 3 / Pro 10 / Enterprise custom); send bundles Starter 500 / Growth 2,000 / Pro 10,000 per month, overage ≈ WhatsApp ₹1.5 · SMS ₹0.35 · email ₹0.10 — canonical numbers at `BM-41`, every one **draft pending rate-card verification** (`BM-17`/`BM-26`) before the meters can be sold. | `BRIEF` — *retired: PRD design note* §2 DD7 ("per-seat price set per market book"), §8 ("marketing sends … bundles per market book"); draft values + included-seat ladder per owner ruling 2026-08-04 | P0 |
...
- *A lead is captured at 11 pm* → capture always works; no unsolicited outbound call before
  ...
  2026-08-04; queue mechanics M02/M07).
...
- Given a recorded, timestamped customer request for a callback at a time outside the window,
  ...
  for that customer (F1-36 lane 3, owner ruling 2026-08-04).
- Given any IN agent call where the customer asks whether they are speaking to an AI, when
  ...
  call does the agent claim to be human (F1-36d hard floors, owner ruling 2026-08-04).
...
`grep -ci 'GST\|DISCOM\|ALMM\|TRAI'` over every `docs/prd/modules/M*.md` must return **0**, with
...
**Foreign-subscriber commercial posture (owner ruling 2026-08-04).** India-first
...
- **No per-tenant compliance exceptions.** No override flag, no support-side bypass of any
  ...
  pass applying owner ruling 2026-08-06; the bullet's rule is unchanged and previously
...
- **No second clock for the messaging floor.** The statutory messaging window is evaluated on the
  ...
  2026-08-06) — there is no per-recipient evaluation, no per-market evaluation alongside it,
  ...
  tenant's clock. *(Non-goal recorded by the pass applying the window's-clock ruling; before it the window's
  clock was open, so no second clock could be excluded.)*
