...
| F1-48 | **Dates and calendar:** date style "12 Mar 2026"; default tenant timezone **Asia/Kolkata** (a default per F1-10); **the IN holiday calendar is authored EMPTY (owner ruling 2026-09-07, Q88)** — TCCCPR states no holiday rule and no row of this suite names a date, so the pack declares an empty calendar rather than a platform-authored list a tenant could never remove (F1-17 permits only narrowing). Every IN holiday is the tenant's own working calendar (M01-59), which the calling window (F1-36) and schedule defaults honor. *(Amended by the pass applying Q88. The row previously read "the **IN holiday calendar** is the pack calendar the calling window (F1-36) and schedule defaults honor" while naming no date anywhere in the suite — a statutory floor with no content — and the clause consuming it at F1-36b cited F1-50, which is metric units. The empty declaration is an authored value in the F1-62a sense, not a missing one, so nothing downstream waits on it.)* | `SRC` — `DOC10.dates-tz` (cited — F3); the empty calendar per owner ruling 2026-09-07 (Q88), on the `F1-62`a authored-empty pattern, with the tenant calendar at `M01-59` | P0 |
...
- **F1-Q2 — RESOLVED (owner ruling 2026-08-04, Q7).** India-first commercially; the first
  ...
  2026-08-04).
- **F1-Q3(a) (register `Q53`) — RESOLVED (owner ruling 2026-08-26).** **The IN pack now declares
  both figures at `F1-62`: an EMPTY messaging window and a 19:00 tenant-local send hour.** The
  research this bullet called for was done against the TCCCPR gazette amendment of 12 Feb 2025:
  it states no hour at all, and its "time band" is a preference each recipient registers with
  their own access provider, binding **promotional traffic only**. The window this bullet waited
  for therefore does not exist to be declared — an empty window IS the authored value, and
  transactional sends are unconstrained. The original statement is kept below for the trail.
  *(Before the ruling:)* **The IN pack declares neither
...
- **F1-Q3(b) (register `Q54`) — RESOLVED (owner ruling 2026-08-06, Q54).** **The hour is read on
  ...
  closed; `Q53` was closed 2026-08-26 by `F1-62`.)*
