...
transactional send channel was **ruled 2026-08-04** and lives with
...
| A newer survey superseded the design's inputs (`design_survey_superseded`) | M05 | The design's author (own) — with the "survey updated — review needed" marker in the studio | ✓ | `SRC` `M05-13` (owner ruling 2026-08-04 — "notifies the designer"); M05 §4 |
...
| F6-21 | **Search is scoped by role visibility — D20, per domain, no leakage.** A result appears only if the searcher's F2 scopes would let them open it: leads per lead visibility, projects per project visibility, people per people-records scope, campaigns per campaign scope (`F2-12`–`F2-14`). Search never becomes the side door around a scope — the dedupe sheet's minimal-disclosure surface (`M02-08`) is the only sanctioned cross-scope reveal in the product, and it is not this one. | `SRC` — `D20` via `F2-12` consumed (authoring-plan Step 4: "scoped by role visibility D20"); `F2-14`/`F2-15`; `M02-08` boundary noted (the dedupe sheet's three-fact disclosure stands untouched) | P0 |
...
- *A rep searches a customer another rep owns* → no result (out of scope); the capture-time
  dedupe sheet — not search — is where the product prevents the double-chase (`M02-08`'s
  three-fact boundary honoured) (F6-21).
...
(owner ruling 2026-08-04; `DOC07.messaging-manual` is the fallback's reference behaviour).
...
timezone (`F1-10`) and never the customer's (`F1-15`, `F1-17`, owner rulings 2026-08-06).
*(Paragraph extended when the crew-message row and its send hour were ruled; it previously
described share-flow requests only, because the registry had no scheduled consumer. The clock
clause was added by the tenant-timezone ruling, which superseded the earlier "customer's market
timezone" reading of the hour.)*
**The key list in F6-26 is the registry's whole seeded key set (owner ruling 2026-08-06),
...
reads one row and no task file. *(Paragraph added by the exhaustive-list ruling; before it
...
- Given each template key, when a share flow requests it in each launch language, then the
  ...
  (F6-26, owner ruling 2026-08-04). *(This line previously read "no send capability
  exists anywhere in the flow", D32's manual-only discipline from before the send-channel
  ruling; it contradicted F6-26's own reconciled row above and is aligned here; the task-side
  copy at `docs/tasks/F-platform.md` T-FPLAT-021 already carries it.)*
- Given the starter set this registry seeds, when it is inspected, then it carries a
  ...
  (F6-26, `F5-68`, `F1-15`, `F1-17`, owner rulings 2026-08-06).
  *(Criterion added with the crew-message row; before that ruling this registry had no row for
  the crew message and no criterion here tested one. The clock clause was added by the
  tenant-timezone ruling.)*
- Given the starter set this registry seeds, when its keys are compared with F6-26's key list,
  ...
  (F6-26, owner ruling 2026-08-06). *(Criterion added by the exhaustive-list ruling; before it
  the list's status was undeclared, `survey_complete` was named only under `docs/tasks/`, and
...
| `modules/M06` / `M07` / `M08` / `M11` | The message-template supply per key and language (F6-26) — every seeded key being named in F6-26's own list, which is exhaustive rather than illustrative (owner ruling 2026-08-06 — clause added when the list was ruled exhaustive, this row previously left the list's status unstated). |
...
| `foundations/F5` | The staff-side landing of every customer-side event (opened, accepted, negotiate, declined, question, call request) — F5 raises them, this document routes them. Plus the seeded `crew_arrival` template row that `F5-68`'s evening-before crew message composes from, and the seeded `survey_complete` row that `F5-14`'s promise-with-a-date composes from (`F6-26`; owner ruling 2026-08-06 — clause added with the crew-message row, this row previously provided events only; `survey_complete` named by the same day's exhaustive-list ruling, which puts it in F6-26's exhaustive key list — the key itself was seeded with the survey-supersession ruling and its emitter is `docs/tasks/M04-survey.md` T-M04-009). |
...
- **No customer-facing sending of its own, and no scheduling of its own** — the matrix is
  ...
  connected channel per the send-channel ruling (owner ruling 2026-08-04), and this document
  defines no channel for them (F6-04). The `crew_arrival` row this registry now seeds (F6-26,
  owner ruling 2026-08-06) does not change that: the copy is this document's, the channel is
  F5/M03's, and the hour is the market pack's floor-bound data, read on the tenant's timezone
  (`F1-10`, `F1-15`, `F1-17`, owner rulings 2026-08-06).
  *(This bullet previously read "the two transactional customer moments"; the count predates
  the ruling that put `F5-68`'s evening-before crew message on the same lane and the act that
  seeds its row here, so it is no longer stated as a count. The rule itself — F6 defines no
  channel and no send moment — is unchanged.)*
...
- **No seeded message key recorded outside F6-26's list.** The list is the registry's whole
  seeded key set, not a sample of it (F6-26, owner ruling 2026-08-06): a task file's DONE
  ...
  (`M01-55`) and are not seeded keys. *(Non-goal recorded by the exhaustive-list ruling; before
  it the list's status was undeclared — the asymmetry this document had recorded as open — so
