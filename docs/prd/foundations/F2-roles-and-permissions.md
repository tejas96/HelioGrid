...
3. **The three rulings `02-personas.md` handed to this document** — design sign-off (`F2-03`), the
   v1 `Manager` fan-out (`F2-08`), and the installer login (`F2-05`) — each decided here with rationale,
   plus this document's half of multi-role composition (`F2-14`; the home half is `M13-10`).
...
|---|---|---|---|
| F2-01 | The preset role set is **exactly twelve fixed presets**, named verbatim for the twelve personas: EPC Owner · Sales Manager · Sales Executive · Survey Engineer · Design Engineer · Project Manager · Field Technician · Installation Team Member · HR/Admin · Finance · Operations · Marketing. Every module and foundation PRD uses these exact names; a document that needs a thirteenth preset raises the need with the owner rather than coining one. | `BRIEF` — `docs/prd/owner-brief-2026-08-03.md` §Users; *retired: PRD design note* §2 DD3 ("~12 roles matching the persona set") · grounded in source at `D27` (fixed presets, journey L1417–1418) | P0 |
...
release adds it without restructuring anything." Recorded as a decision, not an open
question, with that revisit trigger as the stated escape path.
...
- Given the plain payment-request message sent from the tenant's connected official channel,
  ...
  `F2.M11.send-request-message`; owner ruling 2026-08-06).
- Given a tenant with **no** connected official channel, when the same payment-request message is
  ...
  (F2-22, `F2.M11.send-request-message`; owner ruling 2026-08-06 — the log records what the
  product performed, and on that path it performed no send).
...
2026-08-04 ruling (segments-yes, files-no)** replaced the former own-scope-only conservative
...
Marketing can *open*. This is a campaign-domain scope permitting aggregate counts without
record visibility, consistent with the dedupe sheet's minimal-disclosure precedent (`M02-08`,
confirmed by owner ruling: three facts shown, no file access). The Sales
...
owner ruling of 2026-08-04 — the **queuing rep's for their own queued entries**, with
...
Notes on §F2.5-M11 (recorded by Task 19; **amended by owner ruling 2026-08-06**): **three
...
unchanged by that ruling**; the plain *request message*, which carries no instrument, rides the reader's
project scope — **composed, copied and, per owner ruling 2026-08-06, sent from the tenant's
...
and the entry names the person who sent it (owner ruling 2026-08-06)** — `F2-22`'s checklist
...
reconstruction. **The fallback path writes nothing (owner ruling 2026-08-06):** where no
...
the product performed, and the product claims nothing it did not do. No matrix cell moves and no
grant changes. No placeholder remains for M11.
...
(`M13-54`) and grant nothing. The home-composition rule — the home half of the multi-role
question whose permission half `F2-14` resolves — is **resolved by `modules/M13` `M13-10`**: a
fixed preset-precedence ladder over `F2-14`'s domain lattice. No placeholder remains for M13.
...
- The rulings `02-personas.md` handed here: design sign-off (F2-03), the `Manager` fan-out (F2-08),
  the installer login (F2-05), and the permission half of multi-role composition (F2-14).
...
| `modules/M13-dashboards-and-reporting.md` | Role-scoped rendering of every dashboard per F2-12 ("the same screen, scoped"), and the composition rule for multi-role homes (`PS-05`) — the half of multi-role composition this document does not own (`modules/M13` `M13-10`). |
