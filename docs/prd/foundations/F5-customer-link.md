...
- *The pack's crew-message hour falls outside that market's lawful messaging window.* The
  ...
  in force is refused at save (`M01-56`). *(Case added by the pass applying Q50; before the ruling the send hour was undecided and this
...
- Given a scheduled installation, when the customer is informed, then who is coming, when, how
  ...
  criterion here or under `docs/tasks/` tests a send either way." The ruling settles the lane
  ...
  `docs/tasks/F5-customer-link.md` T-F5-002. Row 12 is `registers/conflicts.md`'s owner's to mark
  ...
  declared for IN at `F1-62` since 2026-08-26 — so this criterion tests the resolution rule,
  ...
  both. The window is declared EMPTY for IN at `F1-62` (`Q53`, 2026-08-26), so this
...
- **Register `Q49` — RESOLVED (owner ruling 2026-08-06, Q49), and it is an authoring act
  ...
  side is `docs/tasks/F-platform.md` T-FPLAT-021 (the registry's seed) with
  `docs/tasks/F5-customer-link.md` T-F5-002 (the send). *(This bullet and the next replace the single
...
- **Register `Q53` — STILL OPEN; not decided by `Q54`, not decided by `Q58`, and not decided
  ...
  on, not *what the window is*; nothing in this document, in `docs/tasks/F5-customer-link.md`
  or in `docs/ux/briefs/SCR-F5-02-link-progress.md` assumes an IN hour or an IN messaging window
