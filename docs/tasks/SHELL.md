...
**Type:** screen · **Tier:** P0
**Status:** designed
**Why:** A rep in a customer's living room reaches Add lead with one thumb and a surveyor reaches Start survey the same way, and every person lands on their own work with the company's name in the bar; without it each module invents its own navigation and the two-minutes-to-useful-work promise has nowhere to land.
**PRD rows:** F1-59, F7-22, M01-16, M01-17, M02-06, M07-46, M13-10, MS12-19
...
**Data model:** none — reads `tenant` (the company name in the bar) and `tenant_membership` (the held presets behind the session projection; the first-run coach-mark dismissal state) authored by `T-M01-025` (migration 0002); `user_presence` authored by `T-M07-027` (the availability pill's fact); the preset-precedence ladder `T-M13-006` holds as a product constant; `market_pack_version.pack.dataRights` authored by `T-FCORE-016` (migration 0001), its content by `T-FCORE-009`.
**Contract:** none of its own — it calls:
- GET /auth/session — `sessionProjectionSchema` (`T-M01-025`): the actor, the held presets the ladder and the centre verb resolve from, and the `authorizationVersion` whose rejection renders the access-removed state
- GET /tenants/me — the tenant's name beside the product mark (`T-M01-025`)
- POST /auth/sign-out — the avatar menu's sign-out; work in progress is untouched (`T-M01-025`)
- GET /market-packs/{marketCode} — the `dataRights` key the grievance contact reads (`T-FCORE-016`); the key is absent until `T-FCORE-009` lands, and the menu item renders the unauthored-key state until then
- the coach-mark dismissal write on `tenant_membership` — `T-M01-025` authors the column and declares no route for it; to be declared there
- the availability read and write (available · busy · off, optional until-time) — to be declared by `T-M07-027`
- no composed-home read: the home in force and the presets composed into it derive in `packages/domain` from the session's held presets (`T-M13-006`); each block's content is its owning module's own read, declared with that module (M13-11)
Domain types it reads, adds none: the centre verb and the standing-destination set per preset — `T-FPLAT-025`; the ladder and the home resolution — `T-M13-006`; `ROLE_PRESETS` (`packages/domain/src/authz/`). The shell's own view-model — which home is in force and which body of work is composed in — is a Law 11 type in `packages/domain`, its file settled at `/start`.
**Depends on:** `T-M01-025` (migration 0002 — `tenant`, `tenant_membership`, the guard, `GET /auth/session`, `GET /tenants/me`, `POST /auth/sign-out`) · `T-M01-028` (migration 0005 — the accept whose hand-off lands on this home) · `T-FPLAT-025` (the centre verb and the standing destinations) · `T-M13-006` (the ladder, the home resolution, the composed presets) · `T-M07-027` (`user_presence` — the availability pill ships with that slice, the rest of the shell before it) · `T-FCORE-016` (migration 0001 — the pack read the grievance contact uses) · `packages/ui` `AppShell`, `AppRail`, `TenantHeader`, `Menu`, `CoachMark`, `StatusChip`, `TimeField`, `Wordmark` (landed).
**Out of scope:** the breach-notification duty and the `dataRights` content — `T-FCORE-009`; the verb resolution and the destination set — `T-FPLAT-025`; the ladder, the home resolution and every home's content — `T-M13-006` and the owning modules' home screens; the Quick Add Lead capture screen and its live duplicate check — `T-M02-001`; the routing that reads availability — `T-M07-027`; the atomic accept — `T-M01-028`; the name/photo and role-card screens before this home — `T-M01-009`, `T-M01-010`; the too-old-client refusal and the forced-upgrade screen that replaces this shell (the brief's `update-required` state) — `T-FPLAT-033`; the deactivation behind the access-removed state — `T-FPLAT-003`; the search box's and the bell's own surfaces — `T-SHELL-002`, `T-SHELL-003`; the billing banner this shell hosts — `T-SHELL-006`; sign-in and Google — `T-M01-001`; language and units — `T-M01-011`.
**DONE WHEN:**
- Given a breach event, when duties trigger, then the Data Protection Board and affected principals are notified and the grievance contact is published in-app (F1-59). → proof: qa-parity the avatar menu carries the grievance contact read from the pack's `dataRights` key at 375px and 1536px; the notification duty is proven at `T-FCORE-009`
...
- Given a first-run landing, when coach marks render, then there are at most three, on that screen, each dismissible, and no carousel exists (M01-16). → proof: qa-parity a first sign-in shows at most three `CoachMark` items on the screen landed on, each dismissed on its own and absent after a reload, and no carousel exists on the route
- Given a valid invite, when the invitee verifies the OTP, then user + membership + roles exist atomically and the next screen is name/photo, then the role card, then their role's home with their real assigned work (M01-13, M01-14, M01-17). → proof: qa-parity accept an invite as a Sales Executive with one lead already assigned: after the role card the first screen is My Day inside this shell with that lead listed, never a generic dashboard or a blank; the atomic write is unit-proven at `T-M01-028`
- Given any surface, when the primary add action is used, then quick add opens in one tap and the duplicate check runs live on the phone field (M02-06). → proof: qa-mobile from the home of each preset that may add leads (EPC Owner, Sales Manager, Sales Executive) the centre action opens Quick Add Lead in one tap, and from a Survey Engineer's it opens the survey capture — the verb per home is `T-FPLAT-025`'s; the live duplicate check is proven at `T-M02-001`
- Given a user sets themself off until 3 pm, when routing runs, then they are skipped until then (M07-46). → proof: qa-api set off until 15:00 through the pill: the pill reads Off until 15:00 with its recording line, and `T-M07-027`'s presence read skips the user until then
- Given any combination of held presets, when the person signs in, then their home is the highest-ladder preset's home with every other held preset's today-block composed inside, and a switcher lists each held preset's home (M13-10). → proof: qa-parity sign in holding Sales Executive + Survey Engineer: the home is My Day with today's visits composed in, the title switcher lists both homes, and switching swaps which body of work is the home's own; the derivation is unit-proven at `T-M13-006`
- Given sign-in, Then mobile OTP and Google work and establish tenant/role context with no dead controls (MS12-17); language and units persist per user with real catalogs (MS12-18); sign-out preserves work (MS12-19). → proof: qa-parity sign out from the avatar menu with a draft open: `GET /auth/session` answers no session and the draft is intact on signing back in; the OTP, Google, language and units halves are proven at `T-M01-001` and `T-M01-011`
