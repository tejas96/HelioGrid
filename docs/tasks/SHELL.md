...
- Given any surface, when the primary add action is used, then quick add opens in one tap and the duplicate check runs live on the phone field (M02-06). → proof: qa-mobile from every persona home the centre action opens Quick Add Lead in one tap; the live duplicate check is proven at `T-M02-001`
