...
- the composed-home read (the home in force and every held preset's today-block) — to be declared by `T-M13-006`
...
**Depends on:** `T-M01-025` (migration 0002 — `tenant`, `tenant_membership`, the guard, `GET /auth/session`, `GET /tenants/me`, `POST /auth/sign-out`) · `T-M01-028` (migration 0005 — the accept whose hand-off lands on this home) · `T-FPLAT-025` (the centre verb and the standing destinations) · `T-M13-006` (the ladder, the home resolution, the composed blocks) · `T-M07-027` (`user_presence` — the availability pill ships with that slice, the rest of the shell before it) · `T-FCORE-016` (migration 0001 — the pack read the grievance contact uses) · `packages/ui` `AppShell`, `AppRail`, `TenantHeader`, `Menu`, `CoachMark`, `StatusChip`, `TimeField`, `Wordmark` (landed).
