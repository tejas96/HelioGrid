...
- Given a pack with an unauthored key, when it is stored, then that key is OMITTED from the payload, and on read `unauthoredKeys` names it exactly as it does for an in-memory pack — the seam `market/launch.ts` already holds, and this task is where it stops being hypothetical (`F1-05`).
...
- Given `market_pack` and `market_pack_version`, when the tenancy scan runs, then both pass as readable global reference data — SELECT held, no write privilege, no RLS, for every member of `app_user` — and an injected INSERT grant on either turns the assertion red before the row is cited (`M12`).
- Given the tenant-facing pack read, when any tenant fetches its market's pack, then the response carries no `priceBook` — the book's interior reaches only the billing path (`F1-25`, `BM-17`).
- Given an edited `IN_PACK` literal, when the publish command runs against a market that already holds revision 1, then a second `market_pack_version` exists and the API was not deployed; and given the seed run against that same market, then it reports the difference and writes nothing (`F1-11`).
...
*Out of scope:* any admin authoring screen or API. No PRD row asks for one; the pack stays platform-authored through typed literals and the publish command (`F1-12`).

**DONE WHEN:**

- Given a stored payload whose eight keys are well-formed, when it is parsed, then the result is a `MarketPack` whose every branded value was minted by its owner's constructor, and no `as <Brand>` exists in the parser (`F1-01`, `F1-02`).
- Given a stored payload with one malformed key — a tax rate stored as text, a subsidy slab with a negative ceiling — when it is parsed, then the failure names that key and the path inside it exactly, and nothing partial is returned (`F1-02`).
- Given `PACK_KEYS`, when a key is added to it, then the twin fails to compile until that key's schema exists — the list and the twin cannot drift (`F1-02`).
- Given the first writer that is not typed code — an admin path or a file import — when its slice begins, then this parser already sits on every read of the row, and `T-FCORE-016`'s envelope-only reader is no longer reachable from that path (`F1-11`, `F1-12`).
