...
- Given a pack with an unauthored key, when it is stored, then that key is OMITTED from the payload, and on read `unauthoredKeys` names it exactly as it does for an in-memory pack — the seam `market/launch.ts` already holds, and this task is where it stops being hypothetical (`F1-05`). → proof: unit `packages/domain/tests/market/envelope.test.ts` — `envelopeOf(IN_PACK)` has no `dataRights` property, and `unauthoredKeys` of the pack read back from the row equals `unauthoredKeys(IN_PACK)` (since `T-FCORE-017`, `packages/domain/tests/market/parser.test.ts`)
...
**Ruled at `/start`:** **the owner chose to build it now**, for a second reason the *Due* line does not name: the api reads the CURRENT stored revision (`apps/api/src/modules/market/market.service.ts`), and that row was written by whatever code last ran `pack:publish`. When a later task changes a key's shape, the running code reads an older-shaped row as the new type and a missing field reaches the money path as `undefined`. A row older than the code is as untrusted as a row a hand wrote, so the parser is due now.
**Ruled at `/start`:** **seven keys are stored, not eight.** `dataRights` is not on `MarketPack` until `T-FCORE-009` (parked, block 8) authors it, so `PACK_SCHEMAS` is keyed by the stored keys — `PACK_KEYS` in its order without `dataRights` — and a `dataRights` found in a row is refused as a key with no schema yet. When `T-FCORE-009` adds the key to `MarketPack`, the `Record` type fails to compile until its schema exists.
**Ruled at `/start`:** **one answer, never a throw.** The envelope's own constructors (`marketCode`, `packVersion`) and every brand's constructor report through the same failure — the key (`market` and `version` for the envelope), the path inside it and the constructor's own message — so a caller reads one shape. A property the code does not declare is DROPPED, named in `dropped` and logged by the api, never served; a field the code needs that the row lacks, or a value of the wrong kind, is refused. `ClockTime` is stored as minutes, so its owner gains `clockTimeOfMinutes`, the stored form's door beside `clockTime`'s `HH:MM` one.

**Ruled at `/ship` (owner):** **drop and name, never refuse, a property the code does not declare** — because a release rolls machine by machine (`docs/engineering/09-observability-and-ops.md`), old code reads a row newer code published and new code reads a row older code published; refusing unknown properties would turn every pack shape change into an outage in one direction. The pack publishes in the release step right after the migration, and a pack shape change follows expand/contract: an added field is safe both ways, a removed or renamed field takes two releases. `break-it-reviewer` found the risk (every sign-in reads every market's pack), and the ruling is recorded in the ops doc.
...
**Verified:** digest 65cd1eefc3bc · 2026-09-25 · depth FULL then DELTA · **api 6 pass, 2 inconclusive** · the plan was reviewed first by `break-it-reviewer`, which called six of the first plan's seven steps unable to fail; rewritten · H1 the REAL stored India row (revision 7) passes the strict parser and `GET /market-packs/IN` serves `version` `IN.7`, `tax.platformSale.rateBasisPoints` `1800` and `formats.currency` `INR`, equal to the row · E1 no `priceBook` in the served, parsed payload · N1 an unreached number `422 DOMAIN_RULE_VIOLATION` (regression) · **H2 inconclusive:** the plan named a `currency` field the effective-settings reply does not carry, and the executor's stand-in, the time zone `Asia/Kolkata`, reads the same with or without the parser, so it cannot fail — counted inconclusive by `break-it-reviewer`'s stamp check · **A2 inconclusive: no safe writer** — the only pack writer is the publish command, and a malformed CURRENT India row would break the shared local database; the malformed-row path is proven by the unit suite (`readStoredPack` throws naming every key and path) and, on REAL data, by the stored revision-1 India row written by older code, which the parser refuses naming `tax.registrationTypes.0.format` and `callingRules.messaging.templates` · DELTA after the drop-and-name ruling: D1 revision 7 still passes the whole-number rules (`nsnLength` `10`, `recordRetentionYears` `6`, equal to the row) · D2 no `priceBook` · D3 no false "does not declare" warning in the api log for the clean row · on the wire no step can show the parser's own effect, because a well-formed row reads the same through the old reader; that effect is the unit suite's and the revision-1 evidence's · web, iOS, Android not driven: no screen calls the parser · parity n/a
...
- Given a stored payload with one malformed key — a tax rate stored as text, a subsidy slab with a negative ceiling — when it is parsed, then the failure names that key and the path inside it exactly, and nothing partial is returned (`F1-02`). → proof: unit `parser.test.ts` `it.each` over malformed keys — a text tax rate, a rate above the whole amount, a negative subsidy ceiling, a fractional amount, a clock time past midnight, a calling window that closes before it opens, a market code no market authored, a fractional whole-number length, a missing key — each failure names the key and the path and carries no pack; a second `it.each` drops an undeclared property at the top of the payload, at the top of a key, inside a nested object and inside a label, names each in `dropped`, and serves a pack equal to `IN_PACK`
- Given the stored keys, when a key is added to `MarketPack`, then the twin fails to compile until that key's schema exists — the list and the twin cannot drift (`F1-02`). → proof: unit `parser.test.ts` asserts `Object.keys(PACK_SCHEMAS)` equals `PACK_KEYS` in order without `dataRights`, and each schema is typed to its key's own type, so a schema-less key or a schema whose output differs from the key's type is a `pnpm verify` typecheck failure
