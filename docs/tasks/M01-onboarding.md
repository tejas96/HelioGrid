...
- (**Identity-provider ownership — ruled** (owner ruling 2026-09-07). HelioGrid owns `tenant`, `user_account`, `tenant_membership`, `membership_role`, `invitation` and `invitation_role` (`docs/engineering/data-model.md` §2.1), so the `user + membership + roles exist atomically` write above is a guarded, audited HelioGrid transition (F2-19, F2-20, F2-22). This task's first migration authors `tenant_membership` as a tenant-scoped table — `tenant_id`, a composite index leading with it, a fail-closed RLS policy for `app_user` and explicit grants. Any identity library this task wraps owns ONLY its own tables under its own migrator, and those enter `GLOBAL_TABLES` in `tests/invariants/src/table-tenancy-scan.ts` by their real names with a written reason in the change that lands them; nothing is pre-listed for a library that has not landed.)
