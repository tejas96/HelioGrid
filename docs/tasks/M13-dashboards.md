...
**Data model:** none — reads nothing stored. The held presets arrive in the session projection (`T-M01-025`). The ladder is a product constant in `packages/domain/src/shell/home.ts`, never a table (M13-10: no tenant or runtime writes it; a re-order is a commit), and nothing in preferences chooses a home (M13-09).
**Contract:** none — nothing crosses HTTP. The home in force and the presets composed into it derive from presets the client already holds; no composed-home route exists, because every block's CONTENT is its owning module's own read, declared with that module (M13-11: this module owns placement, never the facts). `docs/tasks/SHELL.md`'s contract list is corrected in this change.
**Depends on:** `T-FPLAT-001` (shipped, #42 — `ROLE_PRESETS`, `inMatrixOrder`) · `T-FPLAT-025` (shipped, #51 — `centreVerbFor(homePreset)` and `standingDestinationsFor(preset)`, the two resolvers this task's output keys).
**Out of scope:** the switcher, the composed home's rendering and its analytics events (home rendered · home switched) — `T-SHELL-001`; every block's content and its own laws — My Day `M07` §M07.1 (`docs/tasks/M07-sales-execution.md`), visits `T-M04-006`, the sign-off queue `T-MS-375`, the field day `T-M09-002`, the installation `T-M08-005`, people today `T-M10-001`, money due `T-M11-001`; each block's teaching empty state (M13-12) and the separate agent block (M13-13, M07's); the first-run hand-off onto the home — `T-M01-010`; the access-removed state when a preset is withdrawn — `T-SHELL-001`, `T-FPLAT-003`.
**Ruled at `/start`:**
- **The ladder is domain's, not contracts'** — the note this ticket carried said contracts, and the precedent it cited says otherwise: the twelve-preset union (`ROLE_PRESETS`) and the UI-language list (`UI_LANGUAGES`) are both AUTHORED in `packages/domain`, and contracts derives a `z.enum` only because those cross HTTP. The ladder never does (`docs/engineering/architecture.md` §4 step 3). It lives in `packages/domain/src/shell/home.ts` beside the two resolvers it feeds — the shell's own view-model file `docs/tasks/SHELL.md` left to be settled here — and not in an M13 folder, because one flow authored in two folders is Law 11 broken.
- **The ladder is an ordered readonly tuple in M13-10's order**, `HOME_LADDER`, held a permutation of `ROLE_PRESETS` by its unit test. The order is data and the rule's shape is the decision (§M13.2 behaviour detail); a re-order is a one-line commit.
- **Three pure functions, all functions of the SET of held presets** (F2-15's "function of the set alone", the `inMatrixOrder` precedent): `homeFor(held)` — the highest-ladder held preset, `null` for no presets (a role-less person is blocked at the guard, F2-21, and the shell never renders); `homesOf(held)` — the switcher's list, each held preset once, in ladder order; `composedHome(held, chosen)` — `{ home, composed }`, the home in force and every OTHER held preset in ladder order, the Law 11 view-model the shell renders: `home` is the body of work that is the home's own, `composed` the presets whose today-blocks sit inside it.
- **A switch is a choice for the session, never stored.** `chosen` is a held preset the person switched to; M13-09 forbids a home preference and v1 keeps no local store (F4-36), so the shell holds it in memory and a fresh sign-in lands on the ladder's home (M01-17). A `chosen` that is not held — a preset withdrawn mid-session — falls back to the ladder's home, never a blank.
- **One entry per held preset in the switcher**, M13-10's literal "the home of every held preset": EPC Owner + Sales Manager lists both, the manager's being the same dashboard team-scoped (M13-30) — a distinct home, not a duplicate. Collapsing two presets onto one surface, if ever wanted, is a rendering decision the switcher makes, not a change here.
- **Block content is composed by each module, placed by the shell.** No content contract of the six blocks exists in code yet (Law 9), and M13-11 makes every one of them its module's own; this task's output names WHICH presets' blocks are inside the home, and the shell mounts each module's own block component over that module's own read. The second done-when line below is M13-11/M13-13's acceptance line: nothing here can alter a block, because `ComposedHome` carries presets only, and each block law is proven where its block lands.
...
(M13-10's surface half — the switcher and the composed home in the app shell — ships in `T-SHELL-001` / `docs/ux/briefs/SCR-SHELL-01-app-shell.md`; this task builds the derivation: the fixed preset-precedence ladder as a product constant, the home in force from the held presets and a switch, the switcher's list, and the composed presets whose today-blocks the shell places inside the home — each block's content from its owning module, unmodified.)
**DONE WHEN:**
- Given any combination of held presets, when the person signs in, then their home is the highest-ladder preset's home with every other held preset's today-block composed inside, and a switcher lists each held preset's home (M13-10). → proof: unit `packages/domain/tests/shell/home.test.ts` — the ladder is a permutation of the twelve; the PRD's own cases resolve as written (rep + surveyor → My Day, Sales Executive; Field Technician + Survey Engineer + Installation Team Member → today's visits, Survey Engineer, with the other two composed in ladder order; EPC Owner who also sells → the owner dashboard with My Day one switch away; Marketing + Sales Executive → campaigns first; one preset → its own home with nothing composed); the answer ignores the order the presets were typed in and a preset typed twice; no presets → `null`; the switcher lists each held preset once in ladder order; a switch to a held preset makes it the home and moves the ladder's home into the composed set; a switch to a preset not held falls back to the ladder's home; the home is never in its own composed set (seen red on a ladder with Sales Executive above Marketing); the switcher on screen and the landing at sign-in are `T-SHELL-001`'s qa-parity proof
- Given a composed home, when its blocks are compared with the owning modules' contracts, then the content is identical and every block law (separate agent block, no-commercial-figures surfaces, gap statements) still holds (M13-11, M13-13). → proof: qa-parity at `T-SHELL-001` and at each block's landing, comparing the composed block with the owning module's own list — nothing in this task holds block content (`ComposedHome` names presets only), and each block law is that block's own proof: the separate agent block with M07's My Day, the no-commercial-figures surface at `T-M08-005` (M13-36), the gap statements at `T-M10-001` (M13-48)
- Given `packages/domain/src/shell/`, when the unit suite runs with coverage, then it stays at 100%. → proof: gate `M71` (the `shell/**` glob, seen red at #51)
...
(The receiving surface is the Notification Center — `T-SHELL-003` / `docs/ux/briefs/SCR-SHELL-03-notification-center.md` carries these verbatim rows; this task builds the monthly summary generation and push scheduling for both summaries, with the two notification types registering with `foundations/F6` per `docs/prd/foundations/F6-notifications-and-search.md`.)
**DONE WHEN:**
...
- **M13-19** (P0) — **Mid-period honesty:** "so far this month" is stated as such; a partial period is never projected or annualised as if it were an actual. — *Enforced by:* the `mid-period-so-far` state in `docs/ux/briefs/SCR-M13-01-owner-dashboard.md` (T-M13-001) and review that no partial-period figure anywhere in T-M13-001…T-M13-005 or T-M13-009 renders projected or annualised.
...
- **M13-32** (P0) → realized-by: `docs/ux/briefs/SCR-M04-06-my-visits-today.md` — SCR-M04-06 screen task (M04 tasks file)
- **M13-33** (P0) → realized-by: `docs/ux/briefs/SCR-MS-02-design-queue.md` — SCR-MS-02 screen task (studio tasks file)
- **M13-34** (P0) → realized-by: `docs/ux/briefs/SCR-M08-01-project-board.md` — SCR-M08-01 screen task (M08 tasks file)
- **M13-35** (P0) → realized-by: `docs/ux/briefs/SCR-M09-02-my-day-route.md` — SCR-M09-02 screen task (M09 tasks file)
- **M13-36** (P0) → realized-by: `docs/ux/briefs/SCR-M08-05-installer-job-home.md` — SCR-M08-05 screen task (M08 tasks file); the F2-06 no-commercial-figure surface law binds T-M13-006's composition of this home — its acceptance line: Given any surface reachable from the Installation Team Member home, when audited, then no price, discount, tranche, margin or customer value appears (M13-36).
- **M13-37** (P0) → realized-by: `docs/ux/briefs/SCR-M10-01-people-today-queue.md` — SCR-M10-01 screen task (M10 tasks file)
- **M13-38** (P0) → realized-by: `docs/ux/briefs/SCR-M11-01-finance-home.md` — SCR-M11-01 screen task (M11 tasks file)
- **M13-40** (P0) → realized-by: `docs/ux/briefs/SCR-M03-01-campaign-list.md` — T-M03-001 (docs/tasks/M03-marketing.md)
- **M13-41** (P0) → realized-by: `docs/ux/briefs/SCR-M07-18-agent-performance.md` — T-M07-018 (docs/tasks/M07-sales-execution.md)
- **M13-42** (P0) → realized-by: `docs/ux/briefs/SCR-M07-18-agent-performance.md` — T-M07-018 (docs/tasks/M07-sales-execution.md)
- **M13-43** (P0) → realized-by: `docs/ux/briefs/SCR-M07-19-call-log.md, docs/ux/briefs/SCR-M07-10-unanswered-questions.md, docs/ux/briefs/SCR-M07-20-agent-usage.md, docs/ux/briefs/SCR-M07-18-agent-performance.md` — T-M07-019, T-M07-010, T-M07-020, T-M07-018 (docs/tasks/M07-sales-execution.md)
- **M13-44** (P1) → realized-by: `docs/ux/briefs/SCR-M07-18-agent-performance.md` — T-M07-018 (docs/tasks/M07-sales-execution.md)
- **M13-46** (P0) → realized-by: `docs/ux/briefs/SCR-M03-01-campaign-list.md` — T-M03-001 (docs/tasks/M03-marketing.md); its acceptance line: Given any campaign figure on any M13 surface or export, when audited, then the caveat is present (M13-46).
- **M13-48** (P0) → realized-by: `docs/ux/briefs/SCR-M10-01-people-today-queue.md` — SCR-M10-01 screen task (M10 tasks file); the non-UI half is a prohibition — no hours-worked, punctuality or people-score computation is built anywhere; its acceptance line: Given a field-day or attendance rollup, when audited, then gaps are stated, unmarked is unmarked, and no score of any kind appears (M13-47, M13-48).
...
| M13-30 | T-M13-001 |
| M13-31 | T-M13-002 |
| M13-32 | realized-by: T-M04-006 — docs/ux/briefs/SCR-M04-06-my-visits-today.md (SCR-M04-06 screen task) |
| M13-33 | realized-by: T-MS-375 (`docs/tasks/MS-studio-c.md`) — brief `docs/ux/briefs/SCR-MS-02-design-queue.md` |
| M13-34 | realized-by: T-M08-001 — docs/ux/briefs/SCR-M08-01-project-board.md (SCR-M08-01 screen task) |
| M13-35 | realized-by: T-M09-002 — docs/ux/briefs/SCR-M09-02-my-day-route.md (SCR-M09-02 screen task) |
| M13-36 | realized-by: T-M08-005 — docs/ux/briefs/SCR-M08-05-installer-job-home.md (SCR-M08-05 screen task) |
| M13-37 | realized-by: T-M10-001 — docs/ux/briefs/SCR-M10-01-people-today-queue.md (SCR-M10-01 screen task) |
| M13-38 | realized-by: T-M11-001 — docs/ux/briefs/SCR-M11-01-finance-home.md (SCR-M11-01 screen task) |
| M13-39 | T-M13-003 |
| M13-40 | realized-by: docs/ux/briefs/SCR-M03-01-campaign-list.md (T-M03-001) |
| M13-41 | realized-by: docs/ux/briefs/SCR-M07-18-agent-performance.md (T-M07-018) |
| M13-42 | realized-by: docs/ux/briefs/SCR-M07-18-agent-performance.md (T-M07-018) |
| M13-43 | realized-by: docs/ux/briefs/SCR-M07-19-call-log.md, SCR-M07-10-unanswered-questions.md, SCR-M07-20-agent-usage.md, SCR-M07-18-agent-performance.md (T-M07-019, T-M07-010, T-M07-020, T-M07-018) |
| M13-44 | realized-by: docs/ux/briefs/SCR-M07-18-agent-performance.md (T-M07-018) |
| M13-45 | T-M13-010 (engine) — surface half T-SHELL-003 |
| M13-46 | realized-by: docs/ux/briefs/SCR-M03-01-campaign-list.md (T-M03-001) |
| M13-47 | T-M13-003 |
| M13-48 | realized-by: T-M10-001 — docs/ux/briefs/SCR-M10-01-people-today-queue.md (SCR-M10-01 screen task) |
| M13-49 | LAW |
| M13-50 | LAW |
| M13-51 | T-M13-011 |
| M13-52 | T-M13-012 |
