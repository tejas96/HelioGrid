...
| Styling | **Tailwind v4** (4.x), full `@import "tailwindcss"` — fresh repo, no legacy cascade layer | Utilities read from `packages/tokens` CSS variables via `@theme`; the POC's legacy-layer gymnastics die here | CSS-in-JS (runtime cost, token bypass risk) |
| Primitives | **Radix UI primitives** (consolidated `radix-ui` 1.x package), styled exclusively with tokens | Unstyled + WAI-ARIA correct: focus trap/restore, roving tabindex — N5 (accessible names, modal focus) comes largely for free | shadcn/ui (vendored styled code with its own token assumptions — fights the Instrument contract) · MUI/Ant (styled systems, theme fights) · Headless UI (thinner component coverage) · React Aria (excellent, but hooks-first model is a bigger authoring tax for agents) |
...
every locale; kW/kWh/kWp are never translated; Noto Sans Devanagari chain with explicit RN
fallback handling. Intl polyfills (`@formatjs/intl-locale`, `@formatjs/intl-pluralrules`) on RN.
