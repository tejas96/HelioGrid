...
3. **City is a text field with suggestions, never a `Select`.** There is no closed list of Indian cities. The value resolves to a **market-pack city**, because `M01-09` matches on company name *and* city, and a steer keyed on free text would fire on *pune* and miss *Pune, MH*. Until the join steer lands (`T-M01-035`) the city is stored as typed; the resolution and its helper arrive with the steer that needs them.
...
7. **The known number is answered after its code verifies, not before** (`M01-08`; the PRD's line reads "when the OTP verifies"). No route says whether a phone has an account, and one that did would let anyone probe which numbers are HelioGrid's. The frame keeps its words and both roads, and the session is held back until the person chooses. The number is shown, not masked — the person just verified it, so no account is disclosed to a stranger.
