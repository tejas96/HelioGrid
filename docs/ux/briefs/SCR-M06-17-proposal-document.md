...
- **MS9-14** (P0) — The 3D moment lives INSIDE the proposal link (`F5-33`) — one link, with the 3D model card, working copy-link feedback and a scannable QR that fails visibly rather than silently (S8-4.8/.12 fix `.41/.53`) (`.49/.51/.52/.55`).
