...
- **missing-language-gap** — a recipient's language has no authored version: the gap is surfaced to the author; nothing is machine-translated, auto-filled or silently substituted (`M03-38` — the missing version shows the original language with a small note, never silent machine translation, M03 §6).
