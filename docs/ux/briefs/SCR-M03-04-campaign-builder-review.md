...
- **window-refused** — the scheduled time falls outside the market's declared messaging window: refused at scheduling with the window named, never silently shifted (`M03-12`). **This state is CONDITIONAL on the market pack declaring a messaging window, and the window text it names is pack data, not screen copy (`F1-15`, `F1-17`; `Q53`).** The IN pack declares no window today — `Q53` is open as a *regulatory-data gap, not a design choice* — so at launch this state does not render for an Indian tenant. Draw it anyway: the design must be complete the day a pack declares one, and the window is evaluated on the **tenant's** timezone (owner ruling 2026-08-06 `Q58`). *(Conditionality added by the readiness pass, 2026-08-07: this state previously read as unconditional, which would have had the designer draw a refusal that cannot fire and invent a window value that does not exist.)*
