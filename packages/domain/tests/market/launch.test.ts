...
describe('unauthoredKeys — which of the eight keys a pack has not authored (F1-02, F1-05)', () => {
  ...
  it('names the one key that is missing', () => {
    expect(unauthoredKeys(packWith(PENDING.filter((key) => key !== 'tax')))).toEqual(['tax']);
  });

  it('lists the missing keys in PRD order, whatever order they were authored in', () => {
    const missingTaxAndBook = PENDING.filter((key) => key !== 'priceBook' && key !== 'tax');
    expect(unauthoredKeys(packWith(missingTaxAndBook))).toEqual(['tax', 'priceBook']);
  });
});
