...
Sources: ledgers `step2-roof-drawing.md` (105 keys) + `step2-roof-ai.md` (78 keys) — 318/318 area tests passing · rulings `step2-roof-rulings.md` (7 rulings covering 14 fixes, S2-1…S2-5.8) · census A.10-3 (42/42 matched across the two halves).
...
| MS2-38 | Capability honesty: the photo-analysis entry appears ONLY when the platform capability is live (S2-4a, M04-21); detection progress narrates three steps (S2-4b, M04-14); per-detection metering rides BM-16/BM-19 (M04-23 contract — POC absence documented at `ai.78`). | `BRIEF` S2-4a/b + law | P0 |
