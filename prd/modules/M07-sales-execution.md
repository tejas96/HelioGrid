...
| M07-20 | **The agent's context is scoped to the single lead matched by the verified caller number; the knowledge base is per-tenant and read-only to the agent; the agent writes nothing beyond the call-outcome record.** No cross-customer retrieval exists. | `SRC` — `DOC08.agent-lead-scope` (docs/08) | P0 |
