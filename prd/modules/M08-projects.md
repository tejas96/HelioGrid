...
| M08-03 | **The project number is server-assigned from tenant counters and never client-generated**, so two people winning deals at the same moment can never collide. This is the same law the proposal number obeys — one identifier rule, two objects (`M06-44` is the proposal half). | `SRC` — `DOC02.server-identifiers` (docs/02 — the project-number half; "Server assigns all business identifiers … never client-generated") | P0 |
