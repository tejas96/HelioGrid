...
`foundations/F8-data-honesty.md`, `_process/2026-08-03-v2-prd-design.md` §2 (DD8–DD12), §9–§10
...
| M01-60 | **Integration credentials are write-only, shown as last-4, probed, and never fail silently.** Tenant-supplied credentials (the BYO payment gateway of `modules/M11`; tenant-side messaging/voice credentials where M07 defines them) display last-4 only after entry; every platform decrypt is audit-logged; credentials are probed on a schedule and an invalid credential raises an alert plus a persistent settings nag — never silent failure. Credentials are the tenant's to rotate. | `SRC` — `DOC08.credentials-last4` (docs/08); `DOC09.credential-probe-nag` (docs/09) | P0 |
...
- **New question raised by the Q51 closure — OPEN, needs a register id.** The confirmed-hard-failure
  ...
  **immediately** in that same state (resend released, voice OTP offered). What the message
  ...
  act and the state's very name (`delivery-failed-retry-later`, `ux/briefs/SCR-M01-01-sign-in.md`)
...
- **Dependent surfaces outside this module, recorded not edited (Q47 application, 2026-08-06).**
  ...
  rail: SMS primary, auto fallback channel on failu…"); and `registers/traceability.md`'s
  `DOC07.otp-delivery` row (typed `live` → `M01-03`, whose automatic-fallback half is now
  superseded). `_process/extraction/docs-rules.md`'s `DOC07.otp-delivery` row is extracted source
