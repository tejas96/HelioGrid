...
| M12-43 | **The billing timers are fixed product timers:** the trial-expiry sweep flips entitlements to the soft-block set (read + export always work); reconciliation runs its 6-hour cadence. Carried as product timers, not calendar language. | `SRC` — `DOC02.trigger-schedule` (docs/02 — the M12 half per Task 17's routing note) | P1 |
