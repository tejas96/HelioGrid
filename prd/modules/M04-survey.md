...
own §5 non-goal on survey-side send machinery. See `prd/registers/conflicts.md` row 4.)*
...
`prd/registers/conflicts.md` row 4.)*
...
- Given a visit that cannot be completed on a tenant with a connected transactional channel, when
  ...
  here — see `prd/registers/conflicts.md` row 4.)*
