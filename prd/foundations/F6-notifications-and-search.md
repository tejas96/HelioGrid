...
the list's status was undeclared and `survey_complete` was recorded only under `tasks/`.)*
...
- Given each template key, when a share flow requests it in each launch language, then the
  ...
  `tasks/F-platform.md` T-FPLAT-021 already carries it.)*
...
- Given the starter set this registry seeds, when its keys are compared with F6-26's key list,
  ...
  ruling the list's status was undeclared, `survey_complete` was named only under `tasks/`, and
