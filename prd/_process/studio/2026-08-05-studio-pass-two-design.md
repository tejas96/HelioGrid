...
Date: 2026-08-05 · Status: owner-approved (this session) · Predecessor: prd/_process/2026-08-03-v2-prd-design.md (DD13)
...
| # | Decision | Ruling |
|---|----------|--------|
| SD1 | Output shape | **Studio sub-suite** at `prd/modules/M05-studio/`: overview/index (current M05 evolves into it) + one document per step/screen + one shared 3D-interaction-model doc. Each doc buildable stand-alone. |
...
2. Inventory keys: `CODE.<area>.<n>` (area = screen or engine slug), one key per feature/
   control/parameter/calculation/edge-case. Ledgers live at `prd/_process/studio/inventory/`.
...
`prd/_process/studio/sittings/` and applied. (c) Defect register populated; declined
...
`prd/_process/studio/` only; no git.
