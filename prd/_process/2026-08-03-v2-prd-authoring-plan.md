...
**Goal:** Author the complete V2 PRD suite (~28 documents under `prd/`) per the approved design spec `prd/_process/2026-08-03-v2-prd-design.md`, closing the §13 completeness ledger against the ~85k-word source corpus.

**Architecture:** Extraction-first. Task 2 converts the entire source corpus into a keyed extraction ledger under `prd/_process/extraction/`; every authoring task consumes its ledger slice and appends traceability rows. Foundations and framing docs land before modules (roles, market packs, honesty rules are referenced everywhere); modules follow journey order; registers close last behind a verification gate that loops until zero unaccounted source items remain.
...
- All product content under `prd/`; process artifacts (this plan, extraction ledger) under `prd/_process/` (DD14).
...
- `REC` items appear in their module **and** in `prd/registers/enhancements.md` with rationale; never mixed with source truth (§5).
...
## Document template (defined once — Task 1 publishes it in `prd/00-README.md`)
...
Traceability row format (appended by every task to `prd/registers/traceability.md`):
...
### Task 1: Suite conventions — `prd/00-README.md`

**Files:**
- Create: `prd/00-README.md`
- Create: `prd/registers/traceability.md`, `prd/registers/conflicts.md`, `prd/registers/enhancements.md`, `prd/registers/open-questions.md` (headers + column definitions only)
...
- [ ] **Step 2: Write `prd/00-README.md`** containing: how-to-read intro; the full doc map (all ~28 files with one-line purposes, copied from spec §4 including the register-naming note); the document template verbatim (from this plan's "Document template" block); tag definitions with the rule "REC never mixed with source truth"; tier definitions (P0 core / P1 important / P2 later — no dates); ID scheme; traceability row format; the reading rules from spec §3 (overlay rule, census rule, conflict rule) so PRD readers know the provenance discipline.
- [ ] **Step 3: Create the four register files** with title, purpose line, and empty table headers: traceability (`source key | disposition | PRD ref`), conflicts (`# | sources involved | contradiction/gap | status: recorded`), enhancements (`REC id | module | recommendation | rationale | tier`), open-questions (`Q id | area | question | decision owner`). Seed conflicts with the two known source gaps (missing `docs/research/*` cited by docs 00/01/03/04/05/12/13/16; deleted `modules/` extractions cited by `docs/product/README.md`). Seed open-questions with `Q1: India price points & bundle sizes need owner re-validation given V2 scope (spec §8)`.
- [ ] **Step 4: Verify:** `grep -c '^| ' prd/00-README.md` ≥ 28 (doc-map rows present); `ls prd/registers/ | wc -l` = 4; conflicts register contains "docs/research" and "modules/"; open-questions contains "re-validation".

### Task 2: Source extraction ledger — `prd/_process/extraction/`
...
**Files:**
- Create: `prd/_process/extraction/d-census.md` — D1–D39: for each, verbatim post-overlay text (apply `docs/15` §2 conformance table + §4 owner directives), status (live/superseded-by-Rn/superseded-by-brief per DD2), and `target_doc` assignment from the mapping table below.
- Create: `prd/_process/extraction/rulings.md` — R1–R18 (docs/15 §1, lines 12–118), R20 + §3 design-system rulings (line 170+), §4 owner directives (line 186+), user-decisions log (line 272+): each with verbatim ruling, consequence, `target_doc`.
- Create: `prd/_process/extraction/journey-stages.md` — Stages 0–8 (journey lines: S0@114, S1@156, S2@190, S3@244, S4@280, S5@418, S6@483, S6B@539, S7@762, S8@837): per stage — screens list, happy path, EVERY "what goes wrong" item (key `S<n>.wrong.<i>`), "deliberately not in v1" items, recommendations, plus "Decisions locked"@33 and "India's calling rules (D36)"@58.
- Create: `prd/_process/extraction/customer-journey.md` — C1–C13 (journey line 950+), the framing rules, link lifecycle.
- Create: `prd/_process/extraction/studio-census-checklist.md` — every census entry from `docs/product/studio-census.md` as a checkbox line (key `SC.<section>.<i>`).
- Create: `prd/_process/extraction/docs-rules.md` — every business rule / product law / product-relevant constraint in docs 00,01,02(product-level only),03(product-relevant constraints only, e.g. unverified-rate-card cautions),04(product rules riding on the model: pinning, archive-never-delete, append-only money),05(domain concepts that surface in product, e.g. fingerprint self-staling; skip pure engineering),06,07(capability level),08(product-visible security/tenancy rules, DPDP),09(product-visible only),10(product-level i18n/design rules),11,14(scope commitments only),16, forward-compat.md, architecture.md(product-level only) — keyed `DOC<nn>.<slug>`.
- Create: `prd/_process/extraction/ux-gaps.md` — UXG-01…UXG-27 from docs/13 with the design-at-implementation notes.
- Create: `prd/_process/extraction/competitive-verdicts.md` — gaps 1–19 from docs/12 with verdicts (key `CG-<n>`), plus the Reslink tier-coverage table and "what nobody else has" moat list.
...
### Task 3: `prd/01-product-overview.md`

**Files:** Create: `prd/01-product-overview.md` · Modify: `prd/registers/traceability.md` (append rows)
...
### Task 4: `prd/02-personas.md`

**Files:** Create: `prd/02-personas.md` · Modify: traceability.
...
- [ ] **Step 1: Read** journey roles matrix (~1425–1465), Stage 1 role-landing content (S1 ledger), docs/08 role matrix rows, brief persona list.
...
### Task 5: `prd/foundations/F2-roles-and-permissions.md`

**Files:** Create: `prd/foundations/F2-roles-and-permissions.md` · Modify: traceability, open-questions.
...
- [ ] **Step 1: Read** ledger: D20/D28/D29 census rows, R16 (installer gap), docs/08 permission rules (`DOC08.*` incl. "Manage catalog and price book" row), journey matrix; design-spec §12 (13 candidates) and DD3/DD11.
...
### Task 6: `prd/foundations/F1-global-market-framework.md`

**Files:** Create: `prd/foundations/F1-global-market-framework.md` · Modify: traceability, conflicts (if pack facts conflict across docs).
...
- [ ] **Step 4: Verify:** 8 pack keys each have ≥1 requirement row; India pack instantiates all 8; `grep -ci 'GST\|DISCOM\|ALMM\|TRAI'` in every `prd/modules/M*.md` authored later must be 0 except pack references — record this as a standing verification rule in the file's cross-module contracts.
- [ ] **Step 5: Append traceability.**

### Task 7: `prd/foundations/F8-data-honesty.md`

**Files:** Create: `prd/foundations/F8-data-honesty.md` · Modify: traceability.
...
### Task 8: `prd/foundations/F3-localization.md`

**Files:** Create: `prd/foundations/F3-localization.md` · Modify: traceability.
...
### Task 9: `prd/foundations/F7-design-language.md`

**Files:** Create: `prd/foundations/F7-design-language.md` · Modify: traceability.
...
### Task 10: `prd/foundations/F4-data-integrity.md`

**Files:** Create: `prd/foundations/F4-data-integrity.md` · Modify: traceability.
...
### Task 11: `prd/04-business-model.md`

**Files:** Create: `prd/04-business-model.md` · Modify: traceability, open-questions, enhancements.
...
### Task 12: `prd/modules/M01-onboarding-and-tenant-config.md`

**Files:** Create: `prd/modules/M01-onboarding-and-tenant-config.md` · Modify: F2 (fill M01 matrix rows), traceability, enhancements.
...
### Task 13: `prd/modules/M02-crm-and-leads.md`

**Files:** Create: `prd/modules/M02-crm-and-leads.md` · Modify: F2 rows, traceability, enhancements.
...
### Task 14: `prd/modules/M04-survey.md`

**Files:** Create: `prd/modules/M04-survey.md` · Modify: F2 rows, traceability.
...
### Task 15: `prd/modules/M05-design-studio.md` (baseline per DD13)

**Files:** Create: `prd/modules/M05-design-studio.md` · Modify: F2 rows, traceability, open-questions.
...
### Task 16: `prd/modules/M06-proposals.md`

**Files:** Create: `prd/modules/M06-proposals.md` · Modify: F2 rows, traceability, enhancements.
...
### Task 17: `prd/modules/M07-sales-execution.md`

**Files:** Create: `prd/modules/M07-sales-execution.md` · Modify: F2 rows, traceability.
...
### Task 18: `prd/modules/M08-projects.md`

**Files:** Create: `prd/modules/M08-projects.md` · Modify: F2 rows, traceability.
...
### Task 19: `prd/modules/M11-payments-and-collections.md`

**Files:** Create: `prd/modules/M11-payments-and-collections.md` · Modify: F2 rows, traceability.
...
### Task 20: `prd/foundations/F5-customer-link.md`

**Files:** Create: `prd/foundations/F5-customer-link.md` · Modify: F2 (link-surface matrix rows), traceability.
...
### Task 21: `prd/modules/M03-marketing.md` (`BRIEF`)

**Files:** Create: `prd/modules/M03-marketing.md` · Modify: F2 rows, traceability, enhancements, conflicts (superseded D-records).
...
### Task 22: `prd/modules/M09-field-workforce.md` (`BRIEF`)

**Files:** Create: `prd/modules/M09-field-workforce.md` · Modify: F2 rows, traceability, enhancements, open-questions.
...
### Task 23: `prd/modules/M10-hr-lite.md` (`BRIEF`) + `prd/modules/M12-platform-billing.md` + `prd/modules/M13-dashboards-and-reporting.md` + `prd/foundations/F6-notifications-and-search.md`
...
### Task 24: `prd/03-journey-map.md`

(Authored late deliberately: it links into finished module PRDs.)

**Files:** Create: `prd/03-journey-map.md` · Modify: traceability.
...
**Files:** Modify: `prd/registers/traceability.md` (close ledger), `prd/registers/conflicts.md`, any PRD doc with defects · Create: `prd/_process/verification-report.md`
...
**Files:** Modify: any PRD doc (fixes), `prd/00-README.md` (final doc map), `prd/foundations/F2-roles-and-permissions.md` (matrix completeness)

- [ ] **Step 1: F2 matrix completeness** — `grep -c 'filled by module task' prd/foundations/F2-roles-and-permissions.md` = 0 (every module filled its rows); every matrix table has 12 role columns.
- [ ] **Step 2: Cross-reference audit** — every `(see <doc>)`/markdown link in `prd/` resolves to an existing file; module §4 contracts reciprocate (if M06 expects the picker from M05, M05's §4 lists it).
- [ ] **Step 3: Template conformance** — every foundation/module doc has all 6 template sections; every requirement row has ID + tag + tier; `grep -rn 'TBD\|TODO' prd/ --include='*.md'` = 0 (open items belong in open-questions register, not inline).
- [ ] **Step 4: Vocabulary sweep** — market-neutrality check (`grep -rn 'GST\|ALMM\|DISCOM\|TRAI\|lakh\|crore\|₹' prd/modules/ prd/foundations/` → hits only as pack references/examples labeled IN-pack, or in F1); role names match F2 exactly; tier names match 04-business-model; "Quotation" appears only in R1 naming notes.
- [ ] **Step 5: Handoff** — update 00-README doc map statuses to `reviewed-pending-owner`; write a short completion summary at the top of `verification-report.md` (docs count, requirement count `grep -rc '^| M[0-9]\|^| F[0-9]' prd/`, SRC/BRIEF/REC counts, open-questions count); present the suite to the owner for review.
