    ...
    coverage: {
      ...
      thresholds: {
        ...
        /* FILES, not the folder: `commerce/`'s remaining two are closed vocabularies whose
           ...
           them buys an import-only test. `meters.ts` earned its bar when `metersBelowCogsFloor`
           became the first thing to READ `METERS`; `states.ts` and `costs.ts` widen this to
           `commerce/**` when theirs land. */
