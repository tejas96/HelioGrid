...
3. **Never modify application source.** The only writable path is `$PWD/.qa/2026-08-02-0042/`.
...
Write every artifact to `$PWD/.qa/2026-08-02-0042/artifacts/` using the exact filename the step declares
...
Write `$PWD/.qa/2026-08-02-0042/report.json` conforming exactly to the schema below. `passes` is an array
...
Instructions:
  ...
  2. after launching, poll with screenshots every 2s (max 30s) until the frame is no longer blank and shows app content; only then capture the declared artifact
  3. screenshot the login screen
  4. tap the phone field, type 9876543210, tap Continue
  5. screenshot the OTP step and count the input boxes
  6. type 123456
  7. wait 4s and screenshot
...
Instructions:
  ...
  2. after launching, poll with screenshots every 2s (max 30s) until the frame is no longer blank and shows app content; only then capture the declared artifact
  3. tap the phone field and type only 98765 (5 digits)
  4. screenshot the Continue button state
...
Instructions:
  ...
  3. tap 'Change number'
  4. screenshot
  5. note whether the previously entered number is still in the field
...
Instructions:
  ...
  2. after launching, poll with `adb exec-out screencap -p` every 2s (max 30s) until the frame no longer shows the 'Loading from' Metro banner and app content is visible; only then capture the declared artifact. A loading frame is `inconclusive`, never a pass
  3. screenshot the login screen with adb exec-out screencap
...
Instructions:
  ...
  2. after launching, poll with `adb exec-out screencap -p` every 2s (max 30s) until the frame no longer shows the 'Loading from' Metro banner and app content is visible; only then capture the declared artifact. A loading frame is `inconclusive`, never a pass
  3. screenshot immediately after launch
