...
| M111 | One task is one PR under the ceiling — at most 25 files or 1,500 lines, one migration, one contract router — split at a layer seam in Law 3's order before the go | — | **NONE** | Review-only: `/start` §3 splits and `/ship` §3 counts the diff and refuses the PR body above the ceiling without the owner's ruling. A script over `git diff --stat` is the obvious holder and lands when the owner asks for one. |
| M112 | The local proof runs in CI's room — a fresh clone of what git would commit, `.env.local` and every git-ignored path absent, the quality lane's `env:` block read from the workflow itself | `scripts/verify-clean.sh` via `pnpm verify:clean`, the proof `/ship` §1 runs | HELD · red 2026-09-10 | Faults that need Linux itself — path case, a tool ubuntu lacks — pass here and fail on GitHub; `act` would hold those and is not installed. |
