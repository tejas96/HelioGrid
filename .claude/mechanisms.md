...
| M17 | pgEnum values equal the contract `z.enum` values | invariant `enum-parity` | HELD · red 2026-09-08 | Seven enums mapped; red on an injected `banned` value in `membership_status`. A new pgEnum must be mapped or excused by name inside the invariant, and both sides are still hand-mirrored from domain's tuple rather than built from it. |
