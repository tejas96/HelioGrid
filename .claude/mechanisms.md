...
| M13 | A cross-tenant read returns zero rows; a cross-tenant write fails; no context fails closed | invariant `tenancy-rls` | HELD · red 2026-09-08 | Seeds two tenants and drives every tenant table and both armed tables as `app_user`. The shape checker runs first, so an injected open policy went red there, before the behavioural loop; the loop is reached only on a canonical shape, and every database-level leak is a shape it sees first. A future tenant table the seed does not populate passes the leak count over zero rows until its seed lands with its migration. |
