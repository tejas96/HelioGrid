...
| M113 | A runtime change is committed only with `/verify`'s stamp for the index's digest, unless it touches no HIGH path (`packages/db`, `packages/contracts`, `packages/data`, `apps/*`, domain's `money`, `tax`, `subsidy`, `pricing`, `authz`, `tranche-allocation.ts`) and the branch names a task (`<kind>/<t-id>-<slug>`) whose STAGED ticket says `**Risk:** LOW` — no task or no Risk line reads as HIGH — and every red proof of the branch's task reads CURRENT against the index; CI reads the stamp and the tier again on every PR head. | git pre-commit and pre-merge-commit (`.claude/hooks/block-unverified-commit.sh` · `scripts/verify-digest.sh --risk`) · CI `quality` |
