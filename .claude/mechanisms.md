...
| M12 | Every table is tenant-scoped (`tenant_id`, a composite index, a fail-closed RLS policy, explicit grants), unreachable global, or readable global reference data (SELECT for every member of `app_user`, no write privilege, no RLS) — each global listed with its written reason | invariant `table-tenancy-scan` · `rls-armed` | PARTIAL · red 2026-09-08 | The readable-global half is proven: an injected `grant insert` on `market_pack` to `app_user` turned it red. The tenant half has zero tenant tables to inspect until `0002`; the checker rejects the known leak shapes and is ready. |
| M13 | A cross-tenant read returns zero rows; a cross-tenant write fails; no context fails closed | invariant `tenancy-rls` | VACUOUS | Zero tenant tables. The guard counts tables carrying `tenant_id`, so global reference data never wakes it; its seed rows and floor are rewritten by the identity spine's task. |
...
| M17 | pgEnum values equal the contract `z.enum` values | invariant `enum-parity` | VACUOUS | Zero pgEnums; the guard keys on `schema_migrations`, so a migrated database with no enum is vacuous, not failed. Structural fix: `db` now imports `domain`, so both sides can be built from the one tuple, as contracts already does. |
