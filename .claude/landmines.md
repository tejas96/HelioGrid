...
| A `str.replace(old, new, 1)` — or any first-match edit — into a long file lands on the FIRST place `old` appears, which in a task file or a ledger is someone else's block, and reports success; a substring match inside a longer line writes a field twice or drops a `*/`. Twice in one session a verify stamp landed in a shipped ticket 400 lines above its own, and lifting it out left a stray blank line that turned four unrelated gates red. | Find the block by its own heading first, edit inside that span, anchor on the whole line, assert the count of matches is one BEFORE writing, and re-read the target after. A ticket is edited in place, never retyped whole. `/ship` §2 diffs every edited task file hunk by hunk. | never |
