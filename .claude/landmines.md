...
| A package's `dist/` and a cached `turbo` result can outlive a change in a package it imports: `pnpm turbo typecheck` has reported every task green over a real type error in `packages/data`, and after `UI_LANGUAGES` grew, `turbo build` left `contracts`' `dist/` typed with the old set. `tsc -b` in these packages lists no `references`. | Delete the `*.tsbuildinfo` files, then `pnpm turbo build --force`; `pnpm verify:clean` has neither to go stale. | the packages declare their workspace imports as `references` (`deferred.md`) |
