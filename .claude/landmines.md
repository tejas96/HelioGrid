...
| A gate's scope was narrowed (tests left out of the verify digest) without moving the review that fired on that scope — a tests-only change would then skip the stamp AND `/ship`'s second actor | the narrowing held back to the slice that adds its compensating check (`break-and-run.sh --stale` at commit, a review trigger that counts tests) | 1 |
