...
| A schema edit by string replacement dropped a doc comment's closing `*/`, turning the next index into comment text; the schema-parity invariant compares tables and columns, not indexes, so nothing went red | the `*/` restored; `schema-parity` now compares indexes too (`M18`), so a lost index goes red | 1 |
...
| A QA step asked its agent to read a fact the agent's tool cannot show (request headers through `read_network_requests`), so it was found only at execution and run again | the step was rewritten to a fact the tool shows | 1 |
| A new gate's parser met an input shape it did not recognise and skipped it in silence, so a malformed block read as well formed | the gate fails on any header variant or stray line it cannot place, and always prints what it read | 1 |
| A number written into a guard from memory rather than read from the data it guards (a corpus floor of 100 rows where there were 98) | the floor replaced by reading every row's status | 1 |
...
| A commit check read the working tree instead of the index the commit writes, so an edit staged and then undone on disk passed | the check reads the index (`--staged`, `--stale --index`, `git grep --cached`), proven by a staged-then-restored case | 1 |
| A comparison was written from the fields its API shows first, not from what the live data uses — index direction was left out while five live indexes are descending | direction, nulls order and method added and proven red; the live data is queried for each property before a comparison's scope is written | 1 |
