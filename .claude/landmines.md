...
| A job failing with no failed step and "The job was not started because it repeatedly failed to be acquired" is GitHub's runner pool, not the code — and the same outage can leave a duplicate run QUEUED on the commit indefinitely, long after its branch merged. | Re-run once; if it recurs, report it. A run still queued on a merged branch is cancelled by hand, with the owner's yes. | never |
