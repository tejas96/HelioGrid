...
This file covers Module M12 (Platform billing): the subscription lifecycle machine and what every state means, entitlements as the product's only runtime gating, the soft-block enforcement gates and the cap ladder, the usage ledger and the usage screen, the dunning ladder and trial nudges, subscription invoicing, refunds and plan changes, the trial's mechanics and conversion, grandfathering mechanics, and the always-available billing screens (Pricing Page, Billing Home, Plan Selection & Conversion, Usage). Task-id prefix: `T-M12-`. Source doc: `prd/modules/M12-platform-billing.md` (rows M12-01…M12-58); the Pricing Page's specification rows (BM-05, BM-07) come from `prd/04-business-model.md` via its brief and are dispositioned under that document's bucket. Screen tasks point at their UX briefs under `ux/briefs/`, where the verbatim requirement rows live; engine, policy and integration tasks quote their rows in full below. The state banner and denial sheets several rows name are SCR-SHELL-06's surface (`tasks/SHELL.md`, brief `ux/briefs/SCR-SHELL-06-billing-state-banner.md`); this file owns the behavior that feeds them.

### T-M12-001 · Pricing Page

**Type:** screen · **Tier:** P0
**PRD rows:** BM-05 (P0), BM-07 (P0) (from `prd/04-business-model.md`; dispositioned under that bucket — listed here as this screen's specification rows)
**DESIGN:** SCR-M12-01 → PENDING
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M12-01-pricing-page.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M12-02-billing-home.md`; they are the specification. (M12-55 is a shared row: its plan-selection half lands on SCR-M12-03 / T-M12-003; the row is dispositioned here.)
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M12-03-plan-selection.md`; they are the specification. (The brief also carries the shared row M12-55 — its plan-selection half — dispositioned under T-M12-002. The trial countdown chip of M12-53 is SCR-SHELL-06's surface, `tasks/SHELL.md`.)
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M12-04-usage-screen.md`; they are the specification. (The brief also carries M12-30's cap-ladder row, dispositioned under T-M12-009 — the ladder is gate machinery; this screen is where its 80% pre-warning must appear.)
**DONE WHEN:**
...
The machine's user-visible face is the state banner (SCR-SHELL-06, `tasks/SHELL.md`); this task owns the states, transitions, timers and append-only history the banner and the billing screens read.
**DONE WHEN:**
...
The closed table M12-23 names, copied verbatim from `prd/modules/M12-platform-billing.md` §M12.4:
...
The typed denial and banner render on SCR-SHELL-06 (`tasks/SHELL.md`, brief `ux/briefs/SCR-SHELL-06-billing-state-banner.md`); this task owns the gates, the cap ladder's timers and the specific pause-message content they emit.
**DONE WHEN:**
