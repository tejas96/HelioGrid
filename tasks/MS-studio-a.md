...
baseline module `prd/modules/M05-design-studio.md` (whose Appendix A census remains the binding
acceptance baseline for the whole studio), plus the three deep-dive step documents
`prd/modules/M05-studio/01-step1-site-setup.md`, `prd/modules/M05-studio/02-step2-roof.md` and
`prd/modules/M05-studio/03-step3-obstructions.md`. Task ids in this file run `T-MS-101` upward.
...
`prd/_process/studio/inventory/file-claims.md` (sitting 1 = Step 1, sitting 2 = Step 2,
sitting 3 = Step 3); every defect whose target requirement is one of this file's rows is attached
to the task that owns that row, from `prd/_process/studio/defect-register.md`.
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-MS-04-step1-site-setup.md`; they are the specification. Every row of this task is present in that brief; no row is quoted here.
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-MS-05-step2-roof.md`; they are the specification. Every row of this task is present in that brief; no row is quoted here.
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-MS-06-step3-obstructions.md`; they are the specification. Every row of this task is present in that brief; no row is quoted here.
...
- MS3-08, MS3-10, MS3-16 and MS3-23 are P1 and carry no line in the document's P0 acceptance block (`prd/modules/M05-studio/03-step3-obstructions.md`, `## Acceptance criteria (P0 coverage)`); they are verified against their brief rows in `ux/briefs/SCR-MS-06-step3-obstructions.md`.
- three base states + brief-listed states present at 375px and 1536px with full parity; zero raw colour literals/off-scale values.
- the ported POC tests for this area pass unchanged in the new project.
...
**DEFECTS:** none registered against MS1-28 or MS1-31 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS1-25, MS1-26 or MS1-27 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS1-24 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS2-13 or MS2-20 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS2-22, MS2-24, MS2-25 or MS2-26 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS2-30 or MS2-33 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS2-40, MS2-41 or MS2-43 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS2-44 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against M05-36, MS3-31 or MS3-34 in `prd/_process/studio/defect-register.md`.
...
**DEFECTS:** none registered against MS3-36, MS3-37 or MS3-39 in `prd/_process/studio/defect-register.md`. (The paired surface defect `CODE.step3-obstructions.32` is attached to T-MS-103 at MS3-28; this task owns the predicate both sides read.)
...
- Given a project containing only some of the six modelled types, When the 3D scene mounts, Then only those types' assets are fetched; and given a corrupt asset or a 404 for a type the project does contain, Then the procedural fallback renders in its place and the scene survives. (Beyond the missing-asset line above, the lazy per-type streaming half and the corrupt/404 half of MS3-38 carry no dedicated Given/When/Then line in `prd/modules/M05-studio/03-step3-obstructions.md`'s P0 acceptance block; the requirement text above is the binding criterion.) The fetch policy is this task's; the mount that triggers it is `3d_design_studio/src/features/solar-studio/three/Scene3D.tsx`, ported by T-MS-206 (`tasks/MS-studio-b.md`), and the fallback's appearance renders on the shared 3D scene (`ux/briefs/SCR-MS-09-3d-scene.md`, same task). Note for the porting engineer: `prd/_process/studio/inventory/step3-obstructions.md` records no test against `.45`/`.46`, so the ported `obstruction-assets.test.ts` pins `.47`/`.48` (MS3-39) only — this pair needs new coverage rather than an inherited net.
- MS3-39 is P1 and carries no separate acceptance line in `prd/modules/M05-studio/03-step3-obstructions.md`; its test-pinned reference dims are the acceptance and travel with the ported suite.
- the ported POC tests for this area pass unchanged in the new project.
...
**PORT:** no POC counterpart — `prd/_process/studio/inventory/file-claims.md` claims no scale-regime files; this extends the ported design payload rather than porting an existing one.
**DEFECTS:** none registered against M05-89 or M05-90 in `prd/_process/studio/defect-register.md`.
...
- Given a 300 kW rooftop design, when opened, then the full per-panel census toolset presents with no scale-motivated reduction — the pure-rooftop half of M05-89, carried by `M05-87`'s acceptance line in `prd/modules/M05-design-studio.md` and by the Appendix A census run (M05-01).
- This task owns the build half of M05-89 — the remove-map scoping that keeps per-panel editing reachable inside a table, and the guarantee that no census entry is dropped when the regime switches. The surface half renders on the layout editor (`ux/briefs/SCR-MS-08-step6-layout-editor.md`, state `block-editing-regime`, T-MS-205 in `tasks/MS-studio-b.md`); it consumes this task's scoping rather than re-deriving it.
...
**Type:** engine · **Tier:** P0
**PRD rows:** M05-13
**PORT:** no POC counterpart — `prd/_process/studio/inventory/file-claims.md` claims no survey-hand-off files; the POC has no survey module and no supersession path at all.
**DEFECTS:** none registered against M05-13 in `prd/_process/studio/defect-register.md`.
...
- The marker is derived by comparison and never hand-set, per M05-10 and `F8-13` — which requires the design to pin the survey version it was built from. **Prerequisite:** M05-10's pinned-input list and `F8-14`'s (T-FPLAT-028, `tasks/F-platform.md`) enumerate the catalog release, price-book version, market-pack/rules version and engine versions only. This task adds the survey version to the design's pinned inputs; without that pin the comparison has nothing to compare and the marker can never fire.
- This task owns the three mechanisms behind the row: (1) the review-needed marker state and the diff it names — which superseding version, and which fields differ in provenance or in value; (2) the notification emit to the design's author — the registered `design_survey_superseded` type in `prd/foundations/F6-notifications-and-search.md`'s matrix, never an ad-hoc one (`F6-05`/`F6-10`; recipient and channel resolve there per `F6-16`, and delivery is T-FPLAT-018's in `tasks/F-platform.md`); (3) the design-side state that gates sending, which the proposal share path reads. Nothing is applied automatically; the designer chooses what to apply, edit by edit, and sent proposals stay pinned (`F8-15`, cited).
- The banner and the persistent marking are drawn by the studio shell — `ux/briefs/SCR-MS-03-studio-shell.md` states `survey-updated-review-needed` and `staleness-marker`, built by T-MS-360 (`tasks/MS-studio-c.md`). The shell renders this task's state; it does not derive it.
- The survey side of the join is M04-66 in T-M04-015 (`tasks/M04-survey.md`) — versioned append and the superseded state. This task is the design-side reconciliation that M04-66 points at.
...
**Type:** engine · **Tier:** P1
**PRD rows:** M05-91
**PORT:** no POC counterpart — `prd/_process/studio/inventory/file-claims.md` claims no scale-regime files; this extends T-MS-116's payload and the ported shading engine rather than porting an existing one. That shading engine is T-MS-207's ported `3d_design_studio/src/features/solar-studio/lib/shading.ts` (MS6-03's measured raycast engine, `tasks/MS-studio-b.md`), given a GPU executor and a permanently retained CPU path, never replaced.
**DEFECTS:** none registered against M05-91 in `prd/_process/studio/defect-register.md`.
...
- The surface half — the block/table sheets and the on-object manipulation — renders on the layout editor (`ux/briefs/SCR-MS-08-step6-layout-editor.md`, T-MS-205 in `tasks/MS-studio-b.md`) and the 3D scene (`ux/briefs/SCR-MS-09-3d-scene.md`, T-MS-206); those screens consume this engine, and the paradigm switch they present is T-MS-116's M05-89.
...
**Type:** engine · **Tier:** P2
**PRD rows:** M05-92
**PORT:** no POC counterpart for the terrain tier — `prd/_process/studio/inventory/file-claims.md` claims no scale-regime files; this extends T-MS-116's payload and T-MS-118's Tier B block model. The winter-solstice shadow-free pitch model this row generalises is T-MS-207's ported `3d_design_studio/src/features/solar-studio/lib/spacing.ts` (MS6-04/MS6-53, `tasks/MS-studio-b.md`), extended to sloped ground at block granularity, never replaced.
**DEFECTS:** none registered against M05-92 in `prd/_process/studio/defect-register.md`.
...
- Given sloped ground, when row pitch is solved, then it is T-MS-207's winter-solstice shadow-free pitch model (`3d_design_studio/src/features/solar-studio/lib/spacing.ts`, MS6-04/MS6-53 in `tasks/MS-studio-b.md`) extended at block granularity — one pitch model shared by flat and sloped ground, extended and never replaced, and the flat-ground result it already produces is unchanged by this task.
...
- Cross-slope tracker articulation is out of scope by §5 of `prd/modules/M05-design-studio.md`, deferred with an explicit trigger; this task builds the single-axis tracker table on a draped mesh and nothing beyond it.
- The surface half renders on the layout editor and the 3D scene (`ux/briefs/SCR-MS-08-step6-layout-editor.md`, `ux/briefs/SCR-MS-09-3d-scene.md` — T-MS-205 and T-MS-206 in `tasks/MS-studio-b.md`); those screens consume this engine.
...
**Type:** engine · **Tier:** P1
**PRD rows:** M05-93
**PORT:** no POC counterpart for the block tier — `prd/_process/studio/inventory/file-claims.md` claims no scale-regime files; this layers above the combiner architecture T-MS-271 (`tasks/MS-studio-b.md`) ports, and does not modify it.
**DEFECTS:** none registered against M05-93 in `prd/_process/studio/defect-register.md`.
...
- The surface half — the scale permit/DXF sheets and the block electrical single-line — renders on the SLD step (`ux/briefs/SCR-MS-11-step8-sld.md`, T-MS-268 in `tasks/MS-studio-b.md`); that screen consumes this engine.
...
- **M05-01** (P0) — **The census is this module's acceptance baseline, incorporated by reference as Appendix A, and it never shrinks.** Every census entry is a binding acceptance item for the studio; the appendix maps every entry to the feature area that carries it. A change that would drop, downgrade or quietly reword a census behaviour is out of order at any time, in this pass or pass two.
  *Enforced by:* the acceptance run of every studio task in this file and its sibling — Appendix A of `prd/modules/M05-design-studio.md` is the checklist; a task may not close having dropped a census behaviour.
...
- **M05-11** (P0) — **A design edited after its proposal exists makes that proposal's pricing visibly stale — the studio's side of the money-never-stale law.** The design surfaces "a proposal was built on an older version of this design" wherever it matters (Done step, BOM, captures review), and the proposal-side staleness display is `modules/M06`'s. Figures inside an already-sent document never move (`F8-15`, cited).
  *Enforced by:* the Done, BOM and captures-review screens (`ux/briefs/SCR-MS-13-done-step.md`, `ux/briefs/SCR-MS-12-step9-bom.md`, `ux/briefs/SCR-MS-10-step7-proposal.md`).
...
- **M05-12** (P0) — **Entitlement checks touch the studio only at save/creation and at proposal Generate — never mid-edit. The flagship is never interrupted per-keystroke, and existing designs always open.** Gate mechanics, ceilings and upgrade prompts are `modules/M12`'s; this module's law is the placement: no entitlement denial ever interrupts editing, blanks a canvas or locks an open design. **The tension is resolved (owner ruling 2026-08-04, Q28): zero feature gates exist in the studio** — the kW ceiling per tier is the **only** gate, enforced exactly at these checkpoints, and over-ceiling designs stay readable forever; the census's no-tier-gate rule holds as law with the ceiling as its single sanctioned boundary.
  *Enforced by:* the shell's Save/Generate paths (`ux/briefs/SCR-MS-03-studio-shell.md`) and review — every studio screen task closes with zero in-surface entitlement checks; MS1-09 in T-MS-101 is the Step-1 instance.
...
- **M05-56** (P2) — **Yield-uncertainty reporting (P50/P90 exceedance) is designed-for as an additive layer on the same energy model** — no second engine, no re-labelling of existing figures; needed when enterprise/utility tenants arrive.
  *Enforced by:* review of the energy-model work behind the energy report (`ux/briefs/SCR-MS-09-3d-scene.md`) — the model must admit an additive exceedance layer; nothing is built for it in v1 (§5 non-goals of `prd/modules/M05-design-studio.md`).
...
- **M05-60** (P0) — **The staleness law binds captures: a capture taken before a design change is stale and must say so — never silently show an out-of-date picture.** This is the studio-side instance of the imagery-honesty law; it rides into the proposal and the customer link with the capture.
  *Enforced by:* the captures/review screen (`ux/briefs/SCR-MS-10-step7-proposal.md`, rows M05-59/M05-61) and the customer surfaces that carry the capture.
...
- **M05-77** (P0) — **The crew sees no money — a work order, not a priced document — on every surface this module emits.** No commercial figure appears on the installation plan or any crew-facing output; the role-surface law is `F2-05`–`F2-07` (cited).
  *Enforced by:* the installation work order screen (`ux/briefs/SCR-MS-17-installation-work-order.md`) and its print output; review rejects any money field on a crew surface.
...
- **M05-80** (P0) — **Exactly one variant per lead may be recommended, and the customer sees exactly one.** Setting `is_recommended` moves the mark, never duplicates it; the customer link and proposal render the recommended variant (their modules' halves).
  *Enforced by:* the design list and variant-compare screens (`ux/briefs/SCR-MS-01-design-list.md`, `ux/briefs/SCR-MS-14-variant-compare.md`).
...
- **M05-81** (P0) — **A customer changing their mind on size is a variant, not a rewrite.** The standing design is preserved; a duplicate is adjusted and compared; the recommendation moves if warranted.
  *Enforced by:* the design list's duplicate/new-variant actions (`ux/briefs/SCR-MS-01-design-list.md`) and review of any in-place resize path.
...
- **M05-82** (P0) — **The rule that does not bend: the product never computes structural adequacy — sign-off is a recorded human decision (who + when), and an unapproved design is never shown to the customer.** The laws are published at `F8-25`/`F8-26`/`F8-28` and are cited, not restated; this module owns the surfaces that make them true.
  *Enforced by:* the sign-off review screen (`ux/briefs/SCR-MS-16-signoff-review.md`), the customer-facing gate on `ux/briefs/SCR-F5-05-customer-3d-view.md`, and the engineer-verification lines this file's own rows carry (MS3-32 in T-MS-103).
...
- **M05-88** (P0) — **Above rooftop scale the editable unit is the block/table/zone — never the panel; panels become derived instances of a table.** Regime is a property of the **design**, not the tenant: a 100 kW rooftop and a 40 MW park coexist in one tenant. The design payload carries blocks alongside the per-panel rooftop model from day one, so 100 MW needs no migration.
  *Enforced by:* T-MS-116, which owns both the payload and row M05-89's census guarantee, and the layout editor's paradigm-switch surface (`ux/briefs/SCR-MS-08-step6-layout-editor.md`, T-MS-205 in `tasks/MS-studio-b.md`).
...
- **MS3-35** (P1) — The obstruction layer is reused READ-ONLY by the layout editor so both steps show identical shapes/chips/rings (`.42`).
  *Enforced by:* the layout editor screen (`ux/briefs/SCR-MS-08-step6-layout-editor.md`) consuming T-MS-103's obstruction layer component unchanged; review rejects a second renderer.

---

## Realized elsewhere

These are `prd/modules/M05-design-studio.md` baseline rows whose surface belongs to a studio step
...
| Row | realized-by |
|---|---|
| M05-02 (P0) | `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` (studio wizard frame) |
| M05-03 (P0) | `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-04 (P0) | `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-05 (P0) | `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-06 (P0) | `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-09 (P0) | `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-21 (P0) | `ux/briefs/SCR-MS-01-design-list.md` + `prd/modules/M05-studio/11-shell-and-platform.md` (design-list entry surface) |
| M05-78 (P0) | `ux/briefs/SCR-MS-01-design-list.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.5 (variant lineage) |
| M05-37 (P0) | `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-38 (P0) | `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-39 (P0) | `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-40 (P0) | `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-41 (P0) | `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-42 (P0) | `prd/modules/M05-studio/04-step4-components.md` row MS4-34 (everything derived recomputes from the store) |
| M05-43 (P0) | `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-44 (P0) | `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-45 (P0) | `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-46 (P0) | `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-47 (P0) | `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-48 (P0) | `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-49 (P0) | `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-50 (P0) | `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-51 (P0) | `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-52 (P0) | `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-53 (P0) | `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-54 (P0) | `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| MS3-40 (P1) | `ux/briefs/SCR-MS-09-3d-scene.md` (per-type obstruction 3D visuals on the shared 3D scene) |
| M05-55 (P0) | `ux/briefs/SCR-F5-05-customer-3d-view.md` + `prd/modules/M05-studio/08-customer-surfaces.md` |
| M05-57 (P0) | `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-58 (P0) | `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-59 (P0) | `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-61 (P0) | `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-62 (P0) | `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-63 (P0) | `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-64 (P0) | `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-65 (P0) | `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-66 (P0) | `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-67 (P0) | `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-68 (P0) | `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-69 (P0) | `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-70 (P0) | `prd/modules/M05-studio/09-step9-bom.md` §MS10.5 row MS10-30 (locked money invariants) |
| M05-71 (P0) | `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-72 (P0) | `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-73 (P0) | `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-74 (P0) | `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-75 (P0) | `ux/briefs/SCR-MS-13-done-step.md` + `prd/modules/M05-studio/10-done-and-installation.md` |
| M05-76 (P0) | `ux/briefs/SCR-MS-17-installation-work-order.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.6 |
| M05-79 (P0) | `ux/briefs/SCR-MS-14-variant-compare.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.5 |
| M05-83 (P0) | `ux/briefs/SCR-MS-15-signoff-queue.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
| M05-84 (P0) | `ux/briefs/SCR-MS-16-signoff-review.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
| M05-85 (P0) | `ux/briefs/SCR-MS-16-signoff-review.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
| M05-86 (P0) | `ux/briefs/SCR-MS-16-signoff-review.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
...
|---|---|
| M05-01 | LAW |
| M05-02 | realized-by: T-MS-360 — `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-03 | realized-by: T-MS-360 — `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-04 | realized-by: T-MS-360 — `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-05 | realized-by: T-MS-360 — `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-06 | realized-by: T-MS-360 — `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-07 | LAW |
| M05-08 | LAW |
| M05-09 | realized-by: T-MS-360 — `ux/briefs/SCR-MS-03-studio-shell.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-10 | LAW |
| M05-11 | LAW |
| M05-12 | LAW |
| M05-13 | T-MS-117 (marker, notification and send-gating state); banner surface realized-by: T-MS-360 — `ux/briefs/SCR-MS-03-studio-shell.md` |
| M05-14 | T-MS-101 |
| M05-15 | T-MS-101 |
| M05-16 | T-MS-101 |
| M05-17 | T-MS-101 |
...
| M05-19 | T-MS-101 |
| M05-20 | T-MS-101 |
| M05-21 | realized-by: T-MS-363 — `ux/briefs/SCR-MS-01-design-list.md` + `prd/modules/M05-studio/11-shell-and-platform.md` |
| M05-22 | T-MS-102 |
| M05-23 | T-MS-102 |
| M05-24 | T-MS-102 |
| M05-25 | T-MS-102 |
...
| M05-35 | T-MS-103 |
| M05-36 | T-MS-114 |
| M05-37 | realized-by: T-MS-201 — `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-38 | realized-by: T-MS-201 — `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-39 | realized-by: T-MS-201 — `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-40 | realized-by: T-MS-201 — `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-41 | realized-by: T-MS-201 — `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-42 | realized-by: T-MS-203 (`tasks/MS-studio-b.md`) — deepened as MS4-34, everything derived recomputes from the store |
| M05-43 | realized-by: T-MS-201 — `ux/briefs/SCR-MS-07-step4-components.md` + `prd/modules/M05-studio/04-step4-components.md` |
| M05-44 | realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-45 | realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-46 | realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-47 | realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-48 | realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-49 | realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-50 | realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-51 | realized-by: T-MS-206 — `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-52 | realized-by: T-MS-206 — `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-53 | realized-by: T-MS-206 — `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-54 | realized-by: T-MS-206 — `ux/briefs/SCR-MS-09-3d-scene.md` + `prd/modules/M05-studio/05-step6-editor.md` |
| M05-55 | realized-by: T-F5-005 — `ux/briefs/SCR-F5-05-customer-3d-view.md` + `prd/modules/M05-studio/08-customer-surfaces.md` |
| M05-56 | LAW |
| M05-57 | realized-by: T-MS-260 — `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-58 | realized-by: T-MS-260 — `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-59 | realized-by: T-MS-260 — `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-60 | LAW |
| M05-61 | realized-by: T-MS-260 — `ux/briefs/SCR-MS-10-step7-proposal.md` + `prd/modules/M05-studio/06-step7-proposal.md` |
| M05-62 | realized-by: T-MS-268 — `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-63 | realized-by: T-MS-268 — `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-64 | realized-by: T-MS-268 — `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-65 | realized-by: T-MS-268 — `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-66 | realized-by: T-MS-268 — `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-67 | realized-by: T-MS-268 — `ux/briefs/SCR-MS-11-step8-sld.md` + `prd/modules/M05-studio/07-step8-sld.md` |
| M05-68 | realized-by: T-MS-301 — `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-69 | realized-by: T-MS-301 — `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-70 | realized-by: T-MS-304 (`tasks/MS-studio-c.md`) — deepened as MS10-30, the locked money invariants |
| M05-71 | realized-by: T-MS-301 — `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-72 | realized-by: T-MS-301 — `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-73 | realized-by: T-MS-301 — `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-74 | realized-by: T-MS-301 — `ux/briefs/SCR-MS-12-step9-bom.md` + `prd/modules/M05-studio/09-step9-bom.md` |
| M05-75 | realized-by: T-MS-310 — `ux/briefs/SCR-MS-13-done-step.md` + `prd/modules/M05-studio/10-done-and-installation.md` |
| M05-76 | realized-by: T-MS-318 — `ux/briefs/SCR-MS-17-installation-work-order.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.6 |
| M05-77 | LAW |
| M05-78 | realized-by: T-MS-363 — `ux/briefs/SCR-MS-01-design-list.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.5 |
| M05-79 | realized-by: T-MS-376 (`tasks/MS-studio-c.md`) — brief `ux/briefs/SCR-MS-14-variant-compare.md`; variant lineage `prd/modules/M05-studio/10-done-and-installation.md` §MS11.5 |
| M05-80 | LAW |
| M05-81 | LAW |
| M05-82 | LAW |
| M05-83 | realized-by: T-MS-312 — `ux/briefs/SCR-MS-15-signoff-queue.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
| M05-84 | realized-by: T-MS-313 — `ux/briefs/SCR-MS-16-signoff-review.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
| M05-85 | realized-by: T-MS-313 — `ux/briefs/SCR-MS-16-signoff-review.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
| M05-86 | realized-by: T-MS-313 — `ux/briefs/SCR-MS-16-signoff-review.md` + `prd/modules/M05-studio/10-done-and-installation.md` §MS11.3 |
| M05-87 | LAW |
| M05-88 | LAW |
| M05-89 | T-MS-116 (census guarantee + remove-map scoping); surface realized-by: T-MS-205 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` |
| M05-90 | T-MS-116 |
| M05-91 | T-MS-118 (Tier B engine); surface realized-by: T-MS-205, T-MS-206 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `ux/briefs/SCR-MS-09-3d-scene.md` |
| M05-92 | T-MS-119 (Tier C engine); surface realized-by: T-MS-205, T-MS-206 — `ux/briefs/SCR-MS-08-step6-layout-editor.md` + `ux/briefs/SCR-MS-09-3d-scene.md` |
| M05-93 | T-MS-120 (block electrical + DXF at scale); surface realized-by: T-MS-268 — `ux/briefs/SCR-MS-11-step8-sld.md` |
| M05-94 | LAW |
| M05-95 | LAW |
| MS1-01 | T-MS-101 |
| MS1-02 | T-MS-101 |
...
| MS3-36 | T-MS-115 |
| MS3-37 | T-MS-115 |
| MS3-38 | T-MS-115 (lazy per-type streaming + procedural fallback); scene surface realized-by: T-MS-206 — `ux/briefs/SCR-MS-09-3d-scene.md` |
| MS3-39 | T-MS-115 |
| MS3-40 | realized-by: T-MS-206 — `ux/briefs/SCR-MS-09-3d-scene.md` |
