...
This file covers module M01 — company signup and authentication, team invites and user onboarding, the role-administration screens, progressive setup and the business profile, the two-tier catalog and price book, branding and document templates, payment-term templates, message templates, capture settings, locale defaults and integration credentials. Task-id prefix: `T-M01-`. Source docs: `prd/modules/M01-onboarding-and-tenant-config.md` (all M01 rows), plus the foreign rows riding M01-owned screens per `prd/registers/screens.md`: `prd/foundations/F2-roles-and-permissions.md`, `prd/foundations/F3-localization.md`, `prd/foundations/F6-notifications-and-search.md`, `prd/foundations/F7-design-language.md`, `prd/modules/M02-crm-and-leads.md`, `prd/modules/M05-studio/11-shell-and-platform.md`. Rules per `tasks/README.md`: acceptance criteria are copied verbatim from the PRD, never rewritten; `DESIGN: PENDING` blocks build, not start.
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-01-sign-in.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-02-company-signup.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-03-onboarding-language.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-04-setup-what-you-sell.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-05-business-profile.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-06-setup-ready.md`; they are the specification.
**DONE WHEN:**
- Given the onboarding sequence, when it runs, then segment + typical size are asked (M01-23), company profile and invites are skippable (M01-24, M01-12), and the final screen offers exactly the two doors (M01-26).
- Three base states + brief-listed states present at 375px and 1536px with full parity; zero raw colour literals/off-scale values.
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-07-invite-teammate.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-08-invite-landing.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-09-first-run-profile.md`; they are the specification.
**DONE WHEN:**
- Given a valid invite, when the invitee verifies the OTP, then user + membership + roles exist atomically and the next screen is name/photo, then the role card, then their role's home with their real assigned work (M01-13, M01-14, M01-17).
- Three base states + brief-listed states present at 375px and 1536px with full parity; zero raw colour literals/off-scale values.
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-10-role-explainer.md`; they are the specification.
**DONE WHEN:**
- Given a valid invite, when the invitee verifies the OTP, then user + membership + roles exist atomically and the next screen is name/photo, then the role card, then their role's home with their real assigned work (M01-13, M01-14, M01-17).
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-11-profile-preferences.md`; they are the specification.
**DONE WHEN:**
- Given any user of any preset, when they open onboarding for the first time and when they open their profile afterwards, then a language picker is available on both platforms, listing each language in its own script and name (`F3-03`).
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-12-team.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-13-assign-roles.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-14-roles-reference.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-15-catalog-settings.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-16-add-catalog-item.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-17-catalog-import-wizard.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-18-branding-settings.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-19-proposal-template-settings.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-20-payment-terms-settings.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-21-message-template-settings.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-22-capture-settings.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-23-locale-defaults.md`; they are the specification.
**DONE WHEN:**
...
**Requirements (verbatim):** Verbatim rows live in `ux/briefs/SCR-M01-24-integration-credentials.md`; they are the specification.
**DONE WHEN:**
...
- **M01-11** (P0) — **Signup contains no plan selection, no billing prompt and no payment instrument.** The trial starts without a card (`04-business-model.md` `BM-28`); every billing surface — plan pick, mandate, invoices, trial state — belongs to `modules/M12-platform-billing.md`. The deferred-era wording of the source ("billing is deferred… no trial gate anywhere") is superseded — billing **is** in v1 (OD-4) — but its signup-shaped consequence survives intact: nothing about money interrupts the front door. — *Enforced by:* the T-M01-002 signup flow containing no plan, billing or payment step (its DONE WHEN line cites M01-11), plus review; billing surfaces are `prd/modules/M12-platform-billing.md`'s.
...
- **M01-29** (P0) — **Configure in context, not in a settings maze.** The moment a person needs a thing that is not configured — a component they stock, a logo about to print, a bank detail about to render — the product offers to set it **there**. "Settings screens exist for revisiting, not for setup." The sharpest instance is the catalog's inline add (M01-39). — *Enforced by:* the prompt-points built in T-M01-005 (first-proposal-send prompt) and T-M01-016 (inline add in-flow), consumer prompt-points in `prd/modules/M06-proposals.md`, and review.
- **M01-30** (P1) — **Every config screen shows the effect.** Live preview is the norm: the proposal with your logo, the agent's opening line spoken aloud (M07 surface), the payment tranches as the customer sees them. — *Enforced by:* the live previews required on T-M01-018, T-M01-019, T-M01-020 and T-M01-021 (their DONE WHEN lines), the M07 test surface (`prd/modules/M07-sales-execution.md`), and review.
...
- **M01-16** (P1) — **First-run coach marks: maximum three, on the screen they actually landed on, dismissible. Never a carousel.**
  *realized-by:* `tasks/SHELL.md` T-SHELL-001 (SCR-SHELL-01; `ux/briefs/SCR-SHELL-01-app-shell.md`). Coach marks render on the shell's role home; SHELL owns the screen task and quotes the row in full.
- **M01-57** (P0) — **Tenant configuration lists the agent & voice surfaces; their behaviour is specified in `modules/M07-sales-execution.md`.** The surfaces: **Agent setup — guided** (name · voice · languages · tone · opening line · what to say when it doesn't know · hand-over rules · calling window, within the floor · free-text "anything else") · **Opening line** (pre-filled disclosure, editable per its floor status) · **Hand-over rules** (editable list; the statutory opt-out is floor) · **Calling window** (days, hours, holiday calendar — narrower than the floor only) · **Business knowledge base** (structured, eight sections, seeded per market — never an empty page; the unanswered-questions one-tap loop) · **Test the agent** ("the most important screen here" — call yourself or run a typed conversation) · **Change history** (versioned config, kept quietly) · **Number provisioning** and **inbound call routing (IVR)** (UXG-16/UXG-17 — M07's slices). M01 owns their presence in the settings information architecture and the M01-28/M01-30 laws applying to them; M07 owns every behaviour.
  *realized-by:* SCR-M07-05 screen task in the M07 tasks file (`ux/briefs/SCR-M07-05-agent-setup-settings.md`; behaviour `prd/modules/M07-sales-execution.md`). M01 owns only the surfaces' presence in the settings information architecture; the register places that surface list on SCR-M07-05.
...
| M01-14 | T-M01-009 |
| M01-15 | T-M01-010 |
| M01-16 | realized-by: tasks/SHELL.md T-SHELL-001 |
| M01-17 | T-M01-025 (non-UI handoff half; surface half tasks/SHELL.md T-SHELL-001) |
| M01-18 | T-M01-025 |
| M01-19 | T-M01-012 |
| M01-20 | T-M01-013 |
| M01-21 | T-M01-014 |
...
| M01-55 | T-M01-021 |
| M01-56 | LAW |
| M01-57 | realized-by: T-M07-005 — ux/briefs/SCR-M07-05-agent-setup-settings.md (SCR-M07-05 screen task, M07 tasks file) |
| M01-58 | T-M01-022 |
| M01-59 | T-M01-023 |
| M01-60 | T-M01-024 |
